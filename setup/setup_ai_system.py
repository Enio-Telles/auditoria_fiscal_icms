    ...
    def check_openai_configuration(self) -> bool:
        ...
        if api_key:
            ...
            try:
                ...
                # Teste simples (verifica acesso listando modelos)
                client.models.list()
...
"""
    ...
    def display_summary(self):
        ...
        # Quebra de linhas longas para atender E501
        dep_status = (
            "Instaladas" if self.requirements_installed else "Verifique manualmente"
        )
        print(
            f"{'✅' if self.requirements_installed else '⚠️'} Dependências Python: {dep_status}"
        )
        ollama_status = "Disponível" if self.ollama_available else "Não configurado"
        print(f"{'✅' if self.ollama_available else '⚠️'} Ollama: {ollama_status}")
        openai_status = "Configurada" if self.openai_available else "Não configurada"
        print(f"{'✅' if self.openai_available else '⚠️'} OpenAI: {openai_status}")
        print("✅ Banco de dados: Inicializado")
        print("✅ Dados de exemplo: Criados")
        print("✅ Scripts de inicialização: Criados")
    ...
    def run_setup(self):
        ...
        try:
            ...
            install_deps = (
                input("\n📦 Instalar dependências Python automaticamente? (s/n): ")
                .lower()
                .strip()
            )
