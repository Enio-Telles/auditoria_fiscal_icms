...
async def check_docker_services():
    ...
    try:
        ...
        pg_result = subprocess.run(
            ["docker", "exec", "auditoria_postgresql", "pg_isready", "-h", "localhost", "-p", "5432"],
            capture_output=True, text=True, timeout=5
        )
        app_state["docker_services"]["postgresql"] = "OK" if pg_result.returncode == 0 else "ERROR"
        
        # Verificar Ollama
        import requests
        try:
            ollama_response = requests.get("http://localhost:11434/api/tags", timeout=3)
            app_state["docker_services"]["ollama"] = "OK" if ollama_response.status_code == 200 else "ERROR"
        except:
...
async def health_check():
    ...
    return {
        ...
        "last_request": app_state["last_request"].isoformat() if app_state["last_request"] else None,
...
async def test_docker():
    ...
    results = {
        "postgresql": {
            "status": app_state["docker_services"]["postgresql"],
            "details": "Container PostgreSQL respondendo" if app_state["docker_services"]["postgresql"] == "OK" else "Falha na conexão"
        },
        "ollama": {
            "status": app_state["docker_services"]["ollama"],
            "details": "API Ollama respondendo" if app_state["docker_services"]["ollama"] == "OK" else "Falha na conexão"
        }
    }
