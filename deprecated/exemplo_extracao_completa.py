...
def exemplo_extracao_completa():
    ...
    if df is not None and not df.empty:
        ...
        print(
            f"   - Registros com descrição válida: {df['descricao_produto'].notna().sum():,}"
        )
        ...
        stats = {
            ...
            "registros_com_ncm": (
                int(df["ncm"].notna().sum()) if "ncm" in df.columns else 0
            ),
            "registros_com_cest": (
                int(df["cest"].notna().sum()) if "cest" in df.columns else 0
            ),
        ...
        stats_filename = (
            f"estatisticas_extracao_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
def exemplo_simulacao_api():
    ...
    print("📋 Resposta da API (test-connection):")
