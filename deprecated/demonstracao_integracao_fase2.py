...
from src.auditoria_icms.data_processing.abc_farma_v2_processor import (
    ABCFarmaV2Processor,
)
...
def demonstrar_integracao_completa():
    ...
    # Instanciar processadores (abc_processor não utilizado diretamente, removido)
    ABCFarmaV2Processor()
    ...
    for i, produto in enumerate(produtos_exemplo, 1):
        ...
        print("🔍 VALIDAÇÃO ESTRUTURA HIERÁRQUICA NCM (Ponto 21):")
        ...
        print(
            f"   Estrutura: Capítulo {estrutura_ncm['estrutura']['capitulo']} → "
        ...
        for regra in resultado_regras["regras_aplicadas"]:
            status = (
                "✅ DEFINIDA" if regra.get("classificacao_definida") else "⏳ PENDENTE"
            )
        ...
        if resultado_cest["aplicavel"]:
            ...
            print(
                f"   Segmento: {resultado_cest['segmento']} - {resultado_cest.get('segmento_nome', '')}"
            )
        ...
        produtos_similares = [
            ...
            if p["PRINCIPIO_ATIVO"] == produto["PRINCIPIO_ATIVO"]
            and p["ID"] != produto["ID"]
        ]
        ...
        if produtos_similares:
            ...
            for similar in produtos_similares:
                ...
                print(
                    f"      Critério: Mesmo princípio ativo ({similar['PRINCIPIO_ATIVO']})"
                )
        else:
            print(
                f"   Nenhum produto similar encontrado para {produto['PRINCIPIO_ATIVO']}"
            )
    ...
    for atividade in atividades_teste:
        ...
        if "porta a porta" in atividade.lower():
            print("      🎯 Segmento especial identificado: 28 (Porta a Porta)")
            print("      📋 Regra: CEST Anexo XXIX prevalece sobre outros anexos")
        elif resultado["aplicavel"]:
            print(
                f"      🎯 Segmento: {resultado['segmento']} - {resultado.get('segmento_nome', '')}"
            )
        else:
            print("      ❌ Produto não se enquadra em CEST específico")
...
if __name__ == "__main__":
    try:
        ...
        print("📁 Processadores disponíveis em:")
        print("   🔸 src/auditoria_icms/data_processing/abc_farma_v2_processor.py")
        print("   🔸 src/auditoria_icms/data_processing/nesh_processor.py")
