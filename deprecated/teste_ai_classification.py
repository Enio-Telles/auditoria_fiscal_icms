...
async def teste_engine_completo():
    ...
    try:
        ...
        for provider in [LLMProvider.HUGGINGFACE, LLMProvider.OLLAMA, LLMProvider.OPENAI]:
            try:
        ...
        try:
            ...
            if ensemble_result.metadata:
                print(f"   Consenso: {ensemble_result.metadata.get('consensus_score', 'N/A')}")
                all_results = ensemble_result.metadata.get('all_results', [])
                print(f"   Modelos usados: {len(all_results)}")
                for model_result in all_results:
                    print(f"     - {model_result['modelo']}: {model_result['ncm']} ({model_result['confianca']:.2f})")
        ...
        for query in queries_teste:
            ...
            if results:
                for i, result in enumerate(results[:3]):
                    print(f"   {i+1}. {result['codigo']}: {result['descricao'][:70]}... (score: {result['score']})")
            else:
