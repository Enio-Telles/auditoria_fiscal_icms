...
class NeshRAGIntegrator:
    ...
    async def _create_optimized_indexes(self):
        ...
        await self.vector_store.create_index(
            "nesh_chapters", filter_type="capitulo_ncm"
        )
        await self.vector_store.create_index(
            "nesh_notes", filter_type="nota_explicativa"
        )
    ...
    async def _update_system_status(self):
        ...
        if status_file.exists():
            ...
            content = content.replace(
                "FALTANDO: 5% de ajustes finais", "SISTEMA 100% OPERACIONAL"
            )
            content = content.replace(
                "Configurar RAG com base NESH 2022 (70% concluído)",
