...
def setup_golden_set_database():
    ...
    # Criar tabelas
    tables = [
        ("golden_set_ncm", golden_ncm_sql),
        ("golden_set_cest", golden_cest_sql)
    ]
...
def insert_sample_empresa(cnpj, razao_social, nome_fantasia, atividade, regime, db_name):
...
def setup_sample_companies():
    ...
    for empresa in empresas:
        ...
        db_name = create_empresa_database(empresa['cnpj_clean'], empresa['razao_social'])
...
def list_databases():
    ...
    list_sql = "SELECT datname FROM pg_database WHERE datistemplate = false ORDER BY datname;"
    ...
    if success:
        ...
        for line in lines:
            if line.strip() and not line.startswith('datname') and not line.startswith('---'):
                db_name = line.strip()
                if 'auditoria' in db_name or 'golden' in db_name or 'empresa' in db_name:
                    print(f"   • {db_name}")
