...
def demo_integracao_completa():
    ...
    if farmaceutico.carregar_dados():
        ...
        print(
            f"   ⚡ {stats_farma['cests_unicos']} CESTs únicos: {stats_farma['cests']}"
        )
    ...
    try:
        ...
        if farma_data and nesh_rules:
            ...
            print(
                f"   📊 {len(farma_data.get('medicamentos', {}))} medicamentos via loader"
            )
            ...
            print(
                f"   🧪 Validação via loader - NCM {ncm_teste}: {validacao_loader['valido']}"
            )
        else:
    ...
    print(
        "🔗 Use StructuredDataLoader para acessar os dados em outras partes do sistema."
    )
...
def demo_caso_uso_pratico():
    ...
    medicamento_ref = farmaceutico.buscar_por_codigo_barras(
        produto_teste["codigo_barras"]
    )
    if medicamento_ref:
        print("   ✅ Produto encontrado na base ABC Farma")
        ...
        print("   ⚠️ Produto não encontrado na base ABC Farma")
    ...
    validacao = nesh.validate_ncm(
        produto_teste["ncm_informado"], produto_teste["descricao"]
    )
