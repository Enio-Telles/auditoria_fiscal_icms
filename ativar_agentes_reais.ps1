...
function Wait-ForService {
    ...
    while ((Get-Date) -lt $timeout) {
        if (Test-Port -Port $Port) {
            Write-Host "[OK] $ServiceName esta respondendo!" -ForegroundColor Green
            return $true
        }
...
function Update-AIConfig {
    ...
    if (Test-Path $aiConfigPath) {
        try {
            ...
            Write-Host "[WARN] Erro ao atualizar configuracao: $($_.Exception.Message)" -ForegroundColor Yellow
        }
    } else {
    ...
    if (Test-Path $configPyPath) {
        try {
            ...
            if ($conteudoPy -notmatch "USE_REAL_AGENTS") {
                ...
                Write-Host "[OK] config.py atualizado" -ForegroundColor Green
            }
        } catch {
            Write-Host "[WARN] Erro ao atualizar config.py: $($_.Exception.Message)" -ForegroundColor Yellow
        }
    }
}
...
function Test-Docker {
    ...
    try {
        ...
        Write-Host "[ERROR] Docker nao esta rodando!" -ForegroundColor Red
    ...
    foreach ($container in $containers) {
        ...
        if (-not $exists) {
            ...
            try {
                ...
                Write-Host "[WARN] Erro ao criar $container" -ForegroundColor Yellow
            }
        } else {
...
function Test-Ollama {
    ...
    try {
        ...
        if (Test-Port -Port 11434) {
            ...
            Write-Host "[ERROR] Ollama nao encontrado!" -ForegroundColor Red
            Write-Host "Instale o Ollama primeiro: https://ollama.ai/download" -ForegroundColor Yellow
            return $false
        }
    } catch {
        Write-Host "[WARN] Erro ao verificar Ollama: $($_.Exception.Message)" -ForegroundColor Yellow
        return $false
    }
}
...
function Test-CondaEnvironment {
    ...
    try {
        ...
        if (-not $condaEnvs) {
            Write-Host "[ERROR] Ambiente conda 'auditoria-fiscal' nao encontrado!" -ForegroundColor Red
            Write-Host "Execute: setup_conda_environment.bat" -ForegroundColor Yellow
            return $false
        }
        Write-Host "[OK] Ambiente conda encontrado" -ForegroundColor Green
        return $true
    } catch {
        Write-Host "[WARN] Erro ao verificar conda: $($_.Exception.Message)" -ForegroundColor Yellow
        return $false
    }
}
    ...
    try {
        ...
        if ($process) {
            ...
            if (Wait-ForService -ServiceName $Nome -Port $Porta -TimeoutSeconds 20) {
                ...
                Write-Host "[WARN] $Nome pode nao ter iniciado corretamente" -ForegroundColor Yellow
                return $false
            }
        }
    } catch {
...
function Test-System {
    ...
    if (Test-Port -Port 11434) {
        try {
            ...
            Write-Host "[OK] Ollama OK" -ForegroundColor Green
        } catch {
    ...
    if (Test-Port -Port 8006) {
        ...
        Write-Host "[WARN] AI Service pode nao estar disponivel" -ForegroundColor Yellow
    }
}
...
try {
    ...
    $aiServiceOK = Start-Microservice -Nome "AI Service" -Caminho "ai-service" -Porta 8006
    Start-Sleep -Seconds 2
