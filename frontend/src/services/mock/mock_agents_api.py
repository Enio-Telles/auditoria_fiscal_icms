...
try:
    from mock_data import (
        get_mock_data, generate_agent, generate_task, 
        generate_workflow, generate_system_metrics, generate_id,
        AGENT_TYPES
    )
except ImportError:
...
def remove_agent(agent_id):
    ...
    agent_index = next((i for i, a in enumerate(mock_data['agents']) if a['id'] == agent_id), None)
...
def get_all_tasks():
    ...
    sorted_tasks = sorted(mock_data['tasks'], 
...
def quick_classify():
    ...
    result = {
        ...
        'errors': [] if random.random() > 0.1 else ['Aviso: Classificação com baixa confiança'],
        'workflow_id': generate_id(),
    }
...
def initialize_system():
    ...
    return jsonify({
        'success': True, 
        'message': 'Sistema de agentes inicializado com sucesso'
    })
...
def shutdown_system():
    ...
    return jsonify({
        'success': True, 
        'message': 'Sistema de agentes desligado com sucesso'
    })
