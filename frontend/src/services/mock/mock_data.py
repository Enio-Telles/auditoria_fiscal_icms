...
def generate_task(agent_id=None):
    ...
    started_at = (
        created_at + timedelta(seconds=random.randint(1, 10))
...
def generate_workflow():
    ...
    return {
        ...
        "current_step": (
            f"step_{random.randint(1, total_steps)}" if status == "running" else None
        ),
        "steps": [
            {
                "id": f"step_{i}",
                ...
                "status": (
                    ...
                    else "failed" if i < completed_steps + failed_steps else "pending"
                ),
                "agent_name": random.choice(list(AGENT_TYPES.keys())),
                "task_type": random.choice(
                    ["classify_ncm", "classify_cest", "expand_description"]
                ),
                "dependencies": [f"step_{j}" for j in range(max(0, i - 1), i)],
                "started_at": (
                    (started_at + timedelta(minutes=i * 5)).isoformat()
                    if started_at and i < completed_steps
                    else None
                ),
                "completed_at": (
                    (started_at + timedelta(minutes=i * 5 + 3)).isoformat()
                    if started_at and i < completed_steps
                    else None
                ),
                ...
                "error": (
                    "Erro na etapa"
                    if i >= completed_steps and i < completed_steps + failed_steps
                    else None
                ),
            }
