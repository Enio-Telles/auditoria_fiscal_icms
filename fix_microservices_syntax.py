...
def fix_service(service_name):
    ...
    try:
        ...
        app_pattern = r'(app = FastAPI\([^)]*\))\s*\n\s*(engine = db_config\.initialize\(\)\s*\nBase\.metadata\.create_all\(bind=engine\)\s*\nlogger\.info\([^)]*\))'
        ...
        content = re.sub(app_pattern, replace_orphaned_db_lines, content, flags=re.MULTILINE | re.DOTALL)
        
        # Fix the lifespan function if it's incomplete
        # Find incomplete lifespan functions and fix them
        incomplete_lifespan = r'@asynccontextmanager\s*\nasync def lifespan\(app: FastAPI\):\s*\n\s*"""Initialize on startup"""\s*\n(.*?)\s*yield\s*\n\s*# Cleanup code \(if needed\) would go here'
...
def main():
    ...
    # Test all services
    import ast
    all_services = ["auth-service", "tenant-service", "product-service", "import-service", "ai-service", "classification-service"]
