...
async def exemplo_agentes_individuais():
    ...
    try:
        ...
        expansion_result = await agent_manager.execute_task(
            "expansion_agent", expansion_task
        )
        print(
            f"Descrição expandida: {expansion_result['expanded_description'][:100]}..."
        )
        ...
        print(
            f"NCM classificado: {ncm_result['ncm_code']} - {ncm_result['ncm_description']}"
        )
        ...
        print(
            f"CEST classificado: {cest_result['cest_code']} - {cest_result['cest_description']}"
        )
        print(f"Confiança: {cest_result['confidence']:.2%}")
        print(
            f"ST Requerida: {'Sim' if cest_result['st_analysis']['st_required'] else 'Não'}"
        )
...
async def exemplo_workflow_completo():
    ...
    try:
        ...
        while True:
            ...
            print(
                f"Status: {status['status']} - Progresso: {status['progress_percentage']:.1f}%"
            )
        ...
        if final_status["status"] == "completed":
            ...
            for step in final_status["steps"]:
                if step["status"] == "completed":
                    print(
                        f"  - {step['id']}: ✓ Concluído em {step['elapsed_time_seconds']:.1f}s"
                    )
                else:
        ...
        print(
            f"  - Total de workflows: {workflow_metrics['overview']['total_workflows']}"
        )
        print(
            f"  - Taxa de sucesso: {workflow_metrics['performance']['success_rate']:.1f}%"
        )
        print(
            f"  - Tempo médio de execução: {workflow_metrics['performance']['average_execution_time_seconds']:.1f}s"
        )
...
async def exemplo_reconciliacao_dados():
    ...
    try:
        ...
        quality_result = await agent_manager.execute_task(
            "reconciler_agent", quality_task
        )
        print(
            f"Score geral de qualidade: {quality_result['overall_quality_score']:.2%}"
        )
        ...
        inconsistency_result = await agent_manager.execute_task(
            "reconciler_agent", inconsistency_task
        )
        total_issues = sum(
            len(inconsistency_result["inconsistencies"][key])
        ...
        print(
            f"Registros originais: {merge_result['merge_summary']['original_records']}"
        )
        print(
            f"Grupos de duplicatas: {merge_result['merge_summary']['duplicate_groups_found']}"
        )
        print(
            f"Registros finais: {merge_result['merge_summary']['final_record_count']}"
        )
...
async def exemplo_classificacao_rapida():
    ...
    for i, produto in enumerate(produtos_teste, 1):
        ...
        if resultado["success"]:
            ...
            print(
                f"   NCM: {summary['ncm_code']} (confiança: {summary['ncm_confidence']:.1%})"
            )
            if summary.get("cest_code"):
                print(
                    f"   CEST: {summary['cest_code']} (confiança: {summary['cest_confidence']:.1%})"
                )
            else:
...
def mostrar_capacidades_sistema():
    ...
    print(
        f"Total de capacidades disponíveis: {sum(len(caps) for caps in capacidades.values())}"
    )
