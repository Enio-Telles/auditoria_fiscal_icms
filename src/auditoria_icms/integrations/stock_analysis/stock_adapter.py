...
class StockAnalysisAdapter(ABC):
    ...
    async def update_classification(
        self, item_id: str, classification: ClassificationResult
    ) -> bool:
...
class GenericStockAdapter(StockAnalysisAdapter):
    ...
    async def _connect_api(self, config: Dict[str, Any]) -> bool:
        ...
        # api_key removido (não utilizado)
    ...
    async def _parse_json_file(
        self, file_path: str, filters: Optional[Dict[str, Any]]
    ) -> List[StockItem]:
    ...
    def _create_stock_item(self, item_data: Dict[str, Any]) -> Optional[StockItem]:
        ...
        try:
            ...
            return StockItem(
                ...
                ncm_current=(
                    str(extracted_data["ncm_current"])
                    if extracted_data["ncm_current"]
                    else None
                ),
                cest_current=(
                    str(extracted_data["cest_current"])
                    if extracted_data["cest_current"]
                    else None
                ),
                supplier=(
                    str(extracted_data["supplier"])
                    if extracted_data["supplier"]
                    else None
                ),
                category=(
                    str(extracted_data["category"])
    ...
    def _item_matches_filters(
        self, item_data: Dict[str, Any], filters: Optional[Dict[str, Any]]
    ) -> bool:
...
class StockIntegrationManager:
    ...
    async def sync_classifications(
        self, system_name: str, results: List[ClassificationResult]
    ) -> bool:
        """
        ...
        for result in results:
            try:
                ...
                self.logger.error(
                    f"Erro ao sincronizar item {result.item_id}: {str(e)}"
                )
