...
class OpenAIClassifier:
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "gpt-3.5-turbo"):
        ...
        if self.api_key:
            try:
                ...
                logger.warning(
                    "Biblioteca openai não instalada. Use: pip install openai"
                )
        else:
            logger.warning("OPENAI_API_KEY não configurada")

    async def classify(
        self, request: ClassificationRequest, knowledge_base: NCMCESTKnowledgeBase
        ...
        similar_ncms = knowledge_base.search_similar_ncm(
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Você é um especialista em classificação fiscal NCM/CEST do Brasil.",
                    },
    ...
    def _build_classification_prompt(
        self, request: ClassificationRequest, similar_ncms: List[Dict]
    ) -> str:
        ...
        context_ncms = (
            ...
            if similar_ncms
            else "Nenhum NCM similar encontrado"
        )
...
"""
    ...
    def _parse_openai_response(
        self, response_text: str, produto_id: str
    ) -> ClassificationResult:
            ...
            if justificativa and len(lines) > 4:
                ...
                for line in lines:
                    if "JUSTIFICATIVA:" in line:
                        start_reading = True
                        justificativa_lines.append(
                            line.replace("JUSTIFICATIVA:", "").strip()
                        )
...
class OllamaClassifier:
    ...
    async def classify(
        ...
        similar_ncms = knowledge_base.search_similar_ncm(
            request.descricao_produto, top_k=3
        )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                parsed_result = self._parse_ollama_response(
                    response_text, request.produto_id
                )
                elapsed = (datetime.now() - start_time).total_seconds()
                parsed_result.tempo_processamento = elapsed
...
class HuggingFaceClassifier:
    ...
    def __init__(self, model_name: str = "microsoft/DialoGPT-medium"):
        ...
        try:
            ...
            logger.warning(
                "Transformers não instalado. Use: pip install transformers torch"
            )
        except Exception as e:
    ...
    async def classify(
        ...
        similar_ncms = knowledge_base.search_similar_ncm(
            request.descricao_produto, top_k=1
        )
...
class AIClassificationEngine:
    ...
    async def classify_single(
        self, request: ClassificationRequest, provider: LLMProvider = LLMProvider.OPENAI
    ) -> ClassificationResult:
    ...
    async def classify_ensemble(
        ...
        valid_results = [
            r
            for r in results
            if isinstance(r, ClassificationResult) and r.ncm_confianca > 0
        ]
        ...
        ensemble_result = ClassificationResult(
            ...
            justificativa=f"Ensemble de {len(valid_results)} modelos. Melhor: {best_result.modelo_usado}. "
            + best_result.justificativa,
            ...
            metadata={
                ...
                "consensus_score": (
                    len(set(r.ncm_sugerido for r in valid_results)) / len(valid_results)
                    if valid_results
                    else 0
                ),
            },
    ...
    async def classify_batch(
        ...
        provider: LLMProvider = LLMProvider.ENSEMBLE,
...
async def classify_product(descricao: str, provider: str = "openai") -> Dict:
    ...
    request = ClassificationRequest(produto_id="single", descricao_produto=descricao)
...
async def classify_products_batch(
    produtos: List[str], provider: str = "ensemble"
) -> List[Dict]:
...
if __name__ == "__main__":
    ...
    print(
        "Use: from ai_classification import classify_product, classify_products_batch"
    )
