...
class IntegratedSystem:
    ...
    def setup_complete_system(self) -> bool:
        ...
        try:
            ...
            if not self._import_external_data():
                self.logger.warning("⚠️ Falha na importação de dados externos (continuando)")
            ...
            for db_name, config in external_config.items():
                try:
                    ...
                    if import_result["success"]:
                        self.logger.info(f"✅ Dados importados de {db_name}: {import_result['products_imported']} produtos")
                    else:
                        self.logger.warning(f"⚠️ Falha na importação de {db_name}: {import_result['error']}")
    ...
    def _validate_agents(self) -> bool:
        ...
        try:
            ...
            cest_test = self.cest_agent.validate_cest("2800100", "84713012", "Processador")
        ...
        try:
            ...
            for required in required_workflows:
                if required not in available_workflows:
                    self.logger.error(f"Workflow obrigatório não disponível: {required}")
                    return False
    ...
    async def process_company_products(self, empresa_id: int, batch_size: int = 100) -> ProcessingResult:
        ...
        try:
            ...
            for i in range(0, len(products), batch_size):
                ...
                self.logger.info(f"🔄 Processando lote {i//batch_size + 1}/{(len(products)-1)//batch_size + 1}")
            ...
            confidence_values = [r.confidence for r in successful_results if r.confidence > 0]
            ...
            return ProcessingResult(
                ...
                confidence_average=sum(confidence_values) / len(confidence_values) if confidence_values else 0,
                manual_review_required=sum(1 for r in results if any(w for w in r.warnings if "manual" in w.lower()))
            )
    ...
    async def _process_product_batch(self, products: List[Dict[str, Any]], empresa_info: Dict[str, Any]) -> List:
        ...
        for product in products:
            try:
                ...
                enrichment_result = self.enrichment_agent.enrich_product_data(product, empresa_info)
                ...
                self.logger.error(f"Erro ao processar produto {product.get('id', 'unknown')}: {e}")
    ...
    async def _save_enriched_product(self, product_id: int, enrichment_result) -> bool:
        ...
        try:
            ...
            manual_review = any("manual" in w.lower() for w in enrichment_result.warnings)
    ...
    async def _log_audit_changes(self, product_id: int, changes: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            return {
                ...
                "summary": {
                    ...
                    "data_completeness": db_stats.get("classificacao_stats", {}).get("com_ncm", 0) / max(db_stats.get("classificacao_stats", {}).get("total_produtos", 1), 1)
                },
                "recommendations": self._generate_recommendations(current_status, db_stats)
            }
    ...
    def _generate_recommendations(self, status: SystemStatus, db_stats: Dict[str, Any]) -> List[str]:
        ...
        if not status.agents_ready:
            recommendations.append("🤖 Verificar configuração e dados dos agentes de classificação")
        ...
        if total_produtos > 0:
            ...
            if ncm_completeness < 0.8:
                recommendations.append("📋 Executar processo de classificação automática para produtos sem NCM")
            
            if requer_revisao > 0:
                recommendations.append(f"👀 Revisar manualmente {requer_revisao} produtos que requerem atenção")
