...
class FiscalAuditWorkflow:
    ...
    def __init__(self, empresa_id: int, config: Optional[Dict[str, Any]] = None):
        ...
        self.workflow_config = {
            ...
            "enable_ncm_classification": self.config.get(
                "enable_ncm_classification", True
            ),
            "enable_cest_classification": self.config.get(
    ...
    def _create_langgraph_workflow(self) -> StateGraph:
        ...
        workflow.add_conditional_edges(
            ...
            {"ncm": "classificacao_ncm", "erro": "tratamento_erro"},
        )
    ...
    def processar_produto(self, produto: ProdutoEmpresa) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Erro no processamento do produto {produto.produto_id}: {str(e)}"
            )
        ...
        try:
            ...
            if self.workflow_config["enable_ncm_classification"]:
                ...
                if resultado_ncm["status"] == "sucesso":
                    ...
                    produto_processado.confianca_ncm = resultado_ncm["resultado"][
                        "confianca"
                    ]
                    produto_processado.justificativa_ncm = resultado_ncm["resultado"][
                        "justificativa"
                    ]
            ...
            if self.workflow_config["enable_cest_classification"]:
                ...
                if resultado_cest["status"] == "sucesso":
                    produto_processado.cest_sugerido = resultado_cest["resultado"][
                        "cest"
                    ]
                    produto_processado.confianca_cest = resultado_cest["resultado"][
                        "confianca"
                    ]
                    produto_processado.justificativa_cest = resultado_cest["resultado"][
                        "justificativa"
                    ]
    ...
    def _node_enriquecimento(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            if resultado["status"] == "sucesso":
                state["descricao_enriquecida"] = resultado["resultado"][
                    "descricao_enriquecida"
                ]
                state["estado_atual"] = ProcessingState.ENRIQUECIDO.value
            else:
    ...
    def _node_classificacao_ncm(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            if resultado["status"] == "sucesso":
                ...
                state["estado_atual"] = ProcessingState.ERRO.value
                state["erro_detalhes"] = resultado.get(
                    "erro", "Erro na classificação NCM"
                )
    ...
    def _node_classificacao_cest(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            if resultado["status"] == "sucesso":
                ...
                state["estado_atual"] = ProcessingState.ERRO.value
                state["erro_detalhes"] = resultado.get(
                    "erro", "Erro na classificação CEST"
                )
    ...
    def _executar_enriquecimento(self, produto: ProdutoEmpresa) -> Dict[str, Any]:
        ...
        try:
            ...
            descricao_enriquecida = (
                f"{descricao_original} - produto enriquecido com análise semântica"
            )
    ...
    def _converter_resultado_langgraph(
        self, resultado_langgraph: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": (
                "sucesso"
                if resultado_langgraph["estado_atual"]
                == ProcessingState.CONCLUIDO.value
                else "erro"
            ),
...
def create_fiscal_workflow(
    empresa_id: int, config: Optional[Dict[str, Any]] = None
) -> FiscalAuditWorkflow:
