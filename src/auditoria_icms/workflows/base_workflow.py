...
class BaseWorkflow(ABC):
    ...
    def __init__(
        self, name: str, config: WorkflowConfig, agents: Dict[str, Any], logger=None
    ):
        self.name = name
        self.config = config
        self.agents = agents
    ...
    async def _process_final_result(self, final_state: WorkflowState) -> Dict[str, Any]:
        ...
        return {
            ...
            "classification": {
                "ncm_final": final_state.get("reconciliation_result", {}).get(
                    "ncm_final"
                ),
                "cest_final": final_state.get("reconciliation_result", {}).get(
                    "cest_final"
                ),
                "justificativa_ncm": final_state.get("reconciliation_result", {}).get(
                    "justificativa_ncm"
                ),
                "justificativa_cest": final_state.get("reconciliation_result", {}).get(
                    "justificativa_cest"
                ),
    ...
    async def enrichment_node(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            result = await enrichment_agent.process(
                input_data, {"empresa_id": state["empresa_id"]}
            )
    ...
    async def cest_classification_node(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            result = await cest_agent.process(
                input_data, {"empresa_id": state["empresa_id"], "estado": "RO"}
            )
