...
class DeterminationFlow(BaseWorkflow):
    ...
    def _enrichment_node(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            self._log_step(
                state, "enrichment", "Iniciando enriquecimento profundo de dados"
            )
                ...
                "candidates": [
                    {
                        "ncm": "30049099",
                        "confidence": 0.82,
                        "justificativa": "Outros medicamentos",
                    },
                    {
                        "ncm": "30049019",
                        "confidence": 0.75,
                        "justificativa": "Outros medicamentos em formas farmacêuticas",
                    },
                    {
                        "ncm": "30049090",
                        "confidence": 0.70,
                        "justificativa": "Medicamentos diversos",
                    },
                ],
            }
        ...
        try:
            ...
            if not ncm_determinado or ncm_result.get("confidence", 0.0) < 0.3:
                self._log_step(
                    state,
                    "cest_determination",
                    "NCM insuficiente para determinação CEST",
                )
            ...
            determination_result = self.cest_agent.determine_cest(
                updated_product_data, state.get("empresa_id"), ncm_context=ncm_result
            )
    ...
    def _reconciliation_node(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            self._log_step(
                state, "reconciliation", "Iniciando reconciliação de determinações"
            )
            ...
            final_result = self.reconciliation_agent.reconcile_determination(
                reconciliation_data, state.get("empresa_id")
            )
    ...
    def _manual_review_node(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self._log_step(
            state, "manual_review", "Determinação marcada para revisão manual"
        )
    ...
    def _get_review_reason(self, state: Dict[str, Any]) -> str:
        ...
        if ncm_result.get("confidence", 0.0) < self.config.confidence_threshold:
            reasons.append(
                f"Determinação NCM incerta ({ncm_result.get('confidence', 0.0):.2f})"
            )

        cest_result = state.get("cest_determination_result", {})
        if cest_result.get("confidence", 0.0) < (
            self.config.confidence_threshold * 0.7
        ):
            reasons.append(
                f"Determinação CEST incerta ({cest_result.get('confidence', 0.0):.2f})"
            )
    ...
    def _suggest_research_areas(self, state: Dict[str, Any]) -> List[str]:
        ...
        if ncm_result.get("confidence", 0.0) < 0.7:
            suggestions.append(
                "Pesquisar especificações técnicas detalhadas do produto"
            )
        ...
        if cest_result.get("confidence", 0.0) < 0.5:
            suggestions.append(
                "Verificar enquadramento no regime de substituição tributária"
            )
            suggestions.append("Consultar convênios CEST específicos do estado")

        if state.get("enrichment_confidence", 0.0) < 0.5:
            suggestions.append(
                "Buscar informações adicionais sobre fabricante/fornecedor"
            )
    ...
    def _get_alternative_classifications(
        ...
        alternatives = {"ncm_candidates": [], "cest_options": []}
