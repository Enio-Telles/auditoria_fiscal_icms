    ...
    def _create_database(self) -> bool:
        ...
        try:
            ...
            if not cursor.fetchone():
                ...
                self.logger.info(
                    f"Banco de dados {self.db_config['database']} já existe"
                )
                    ...
                    ncm_data.append(
                        {
                            ...
                            "aliquota_ii": (
                                float(row.get("Alíq. II (%)", 0))
                                if pd.notna(row.get("Alíq. II (%)"))
                                else 0
                            ),
                            "aliquota_ipi": (
                                float(row.get("Alíq. IPI (%)", 0))
                                if pd.notna(row.get("Alíq. IPI (%)"))
                                else 0
                            ),
                        }
                    )
                        ...
                        cest_records.append(
                            {
                                ...
                                "ncm_vinculados": (
                                    [item.get("ncm", "")] if item.get("ncm") else []
                                ),
    ...
    def _setup_indexes(self) -> bool:
        ...
        try:
            ...
            CREATE INDEX IF NOT EXISTS idx_ncm_descricao
                ON ncm_classificacoes USING gin(
