...
class Classificacao(Base):
    ...
    id = Column(Integer, primary_key=True)
    mercadoria_id = Column(
        Integer, ForeignKey("mercadorias_a_classificar.id"), nullable=False
    )
...
class Agregacao(Base):
    ...
    __table_args__ = (
        Index(
            "idx_agregacao_empresa_conceitual", "empresa_id", "produto_conceitual_id"
        ),
    )
...
class ConfiguracaoProcessamento(Base):
    ...
    __table_args__ = (Index("idx_config_empresa", "empresa_id"),)
...
class StatusProcessamento(Base):
    ...
    status = Column(
        String(50), default="iniciado"
    )  # iniciado, em_progresso, concluido, erro, cancelado
...
class NCM(Base):
    ...
    __table_args__ = (Index("idx_ncm_hierarquia", "capitulo", "posicao", "subposicao"),)
...
class NCMCestAssociacao(Base):
    ...
    ncm_codigo_completo = Column(
        String(8), ForeignKey("ncm.codigo")
    )  # Para NCM específico
    tipo_associacao = Column(
        String(20), default="especifico"
    )  # especifico, padrao, hierarquico
...
class ProdutoExemplo(Base):
    ...
    __table_args__ = (Index("idx_produto_ncm_cest", "ncm", "cest"),)
...
class LogAgente(Base):
    ...
    __table_args__ = (Index("idx_log_agente_timestamp", "agente_nome", "timestamp"),)
...
class ProdutoEmpresa(Base):
    ...
    produto_id = Column(
        String(100), primary_key=True, comment="ID único do produto na empresa"
    )
    empresa_id = Column(
        Integer,
        ForeignKey("empresas.id"),
        nullable=False,
        comment="ID da empresa no sistema",
    )
    codigo_produto = Column(
        String(50), nullable=False, comment="Código do produto na empresa"
    )
    descricao_produto = Column(
        Text, nullable=False, comment="Descrição original do produto"
    )
    codigo_barra = Column(
        String(20), nullable=True, comment="Código de barras do produto"
    )

    # Classificações originais da empresa
    ncm = Column(
        String(8), nullable=True, comment="NCM original informado pela empresa"
    )
    cest = Column(
        String(10), nullable=True, comment="CEST original informado pela empresa"
    )

    # Resultados do processamento dos agentes
    descricao_enriquecida = Column(
        Text, nullable=True, comment="Descrição enriquecida pelos agentes"
    )
    ncm_sugerido = Column(
        String(8), nullable=True, comment="NCM sugerido pelos agentes"
    )
    cest_sugerido = Column(
        String(10), nullable=True, comment="CEST sugerido pelos agentes"
    )

    # Métricas de confiança
    confianca_ncm = Column(
        Float, nullable=True, comment="Confiança da classificação NCM (0.0 a 1.0)"
    )
    confianca_cest = Column(
        Float, nullable=True, comment="Confiança da classificação CEST (0.0 a 1.0)"
    )

    # Justificativas dos agentes
    justificativa_ncm = Column(
        Text, nullable=True, comment="Justificativa da classificação NCM"
    )
    justificativa_cest = Column(
        Text, nullable=True, comment="Justificativa da classificação CEST"
    )
    ...
    revisao_manual = Column(
        Boolean, default=False, comment="Indica se requer revisão manual"
    )

    # Campos de controle
    data_criacao = Column(
        DateTime, default=func.now, comment="Data de criação do registro"
    )
    ...
    data_processamento = Column(
        DateTime, nullable=True, comment="Data do último processamento pelos agentes"
    )
    ...
    def __repr__(self):
        return (
            "<ProdutoEmpresa(produto_id='"
            f"{self.produto_id}', codigo='"
            f"{self.codigo_produto}', empresa_id={self.empresa_id})>"
        )
