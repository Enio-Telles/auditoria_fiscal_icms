...
class LocalLLMClassifier:
    ...
    def query_ollama(self, prompt: str, model: str = None) -> Optional[str]:
        ...
        for attempt in range(self.config.max_retries):
            try:
                logger.info(f"Consultando Ollama (tentativa {attempt + 1}) - modelo: {model}")
...
"""
    ...
    def classify_product(self, produto_id: str, descricao: str) -> ClassificationResult:
        ...
        return ClassificationResult(
            ...
            justificativa=classification.get("justificativa", "Classificação automática"),
    ...
    def batch_classify(self, produtos: List[Dict[str, str]]) -> List[ClassificationResult]:
        ...
        for i, produto in enumerate(produtos, 1):
            logger.info(f"Classificando produto {i}/{total}: {produto.get('descricao', '')[:50]}...")
            
            result = self.classify_product(
                produto.get('produto_id', f"prod_{i}"),
            ...
            # Log do resultado
            logger.info(f"  → NCM: {result.ncm_sugerido} (conf: {result.ncm_confianca:.2f})")
            if result.cest_sugerido:
                logger.info(f"  → CEST: {result.cest_sugerido} (conf: {result.cest_confianca:.2f})")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--install-models", action="store_true", help="Instalar modelos recomendados")
    ...
    if args.install_models:
        ...
        for produto in produtos_teste:
            result = classifier.classify_product(produto["produto_id"], produto["descricao"])
            print(f"\n📱 {produto['descricao']}")
            print(f"   NCM: {result.ncm_sugerido} (conf: {result.ncm_confianca:.2f})")
            print(f"   CEST: {result.cest_sugerido} (conf: {result.cest_confianca:.2f})")
