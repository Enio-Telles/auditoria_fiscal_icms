...
class ProdutoClassificacao(BaseModel):
    ...
    contexto_adicional: Optional[str] = Field(None, description="Informações adicionais")
...
class ClassificacaoLote(BaseModel):
    ...
    provider: Optional[str] = Field("ensemble", description="Provedor de IA: openai, ollama, huggingface, ensemble")
...
def convert_to_classification_request(produto: ProdutoClassificacao) -> ClassificationRequest:
...
def convert_from_classification_result(result: ClassificationResult) -> ResultadoClassificacao:
...
async def get_ai_status():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao verificar status da IA: {str(e)}")
...
async def classify_single_product(
    ...
    if cache_key in classification_cache:
        logging.info(f"Resultado encontrado no cache para: {produto.descricao_produto[:50]}")
    ...
    try:
        ...
        logging.info(f"Produto classificado: {produto.descricao_produto[:50]} -> NCM {result.ncm_sugerido}")
        return response
...
async def classify_products_batch_endpoint(
    ...
    try:
        ...
        classification_requests = [
            convert_to_classification_request(produto) 
        ...
        logging.info(f"Lote de {len(request.produtos)} produtos: {len(cached_results)} no cache, {len(pending_requests)} para processar")
        ...
        if pending_requests:
            ...
            if request.provider == "ensemble":
                results = await engine.classify_batch(pending_reqs, LLMProvider.ENSEMBLE)
            else:
        ...
        for i in range(len(request.produtos)):
            if i in cached_results:
                ...
                final_results.append(ResultadoClassificacao(
        ...
        raise HTTPException(status_code=500, detail=f"Erro na classificação em lote: {str(e)}")
...
async def ai_demo():
    ...
    for produto in produtos_demo:
        try:
            result = await classify_product(produto, "huggingface")  # Usar HF por ser mais rápido
            results.append({
                "produto": produto,
                "ncm": result['ncm'],
                "confianca": result['confianca'],
