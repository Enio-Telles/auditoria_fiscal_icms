...
from ...database.models import Usuario, Empresa, ProdutoEmpresa as MercadoriaAClassificar
...
async def execute_data_import(
    ...
    try:
        ...
        import_jobs_cache[job_id]["message"] = "Conectando ao banco de dados da empresa..."
        ...
        # Atualizar status
        import_jobs_cache[job_id]["message"] = "Executando consulta no banco da empresa..."
        ...
        if not records:
            ...
            import_jobs_cache[job_id]["message"] = "Nenhum registro encontrado para importar"
        ...
        import_jobs_cache[job_id]["message"] = f"Importando {total_records} registros..."
        
        logger.info(f"📦 Importando {total_records} registros para empresa {empresa_id}")
            ...
            for i in range(0, total_records, batch_size):
                ...
                for record in batch:
                    ...
                    mercadoria = MercadoriaAClassificar(
                        ...
                        id_agregados=record.get("id_agregados", record.get("produto_id")),
                        status="PENDENTE"
                    )
                ...
                import_jobs_cache[job_id]["message"] = f"Processado {processed}/{total_records} registros ({progress:.1f}%)"
        ...
        import_jobs_cache[job_id]["message"] = f"Importação concluída com sucesso! {processed} registros importados."
...
@router.post("/import", response_model=DataImportResponse, summary="Importar dados da empresa")
async def import_company_data(
    ...
    """
    logger.info(f"📥 Solicitação de importação para empresa {import_request.empresa_id} - Por: {current_user.email}")
    ...
    if not access:
        logger.warning(f"❌ Usuário {current_user.email} sem acesso à empresa {import_request.empresa_id}")
        raise HTTPException(
    ...
    empresa = db.query(Empresa).filter(
        Empresa.id == import_request.empresa_id,
        Empresa.ativo == True
    ).first()
...
@router.delete("/import/{job_id}", response_model=MessageResponse, summary="Cancelar importação")
