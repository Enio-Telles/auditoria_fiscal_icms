...
async def list_users(
    ...
    if search:
        ...
        query = query.filter(
            or_(Usuario.nome.ilike(search_term), Usuario.email.ilike(search_term))
        )
...
async def get_user(
    ...
    """
    logger.info(
        f"👤 Buscando usuário ID {user_id} - Solicitado por: {current_user.email}"
    )
...
async def create_user(
    ...
    if existing_user:
        ...
        raise BusinessLogicError(
            "Email já cadastrado no sistema", {"email": user_data.email}
        )
    ...
    try:
        ...
        logger.info(
            f"✅ Usuário criado com sucesso: {new_user.email} (ID: {new_user.id})"
        )
        return new_user
...
@router.delete(
    "/{user_id}", response_model=MessageResponse, summary="Desativar usuário"
)
...
@router.post(
    "/{user_id}/activate", response_model=MessageResponse, summary="Reativar usuário"
)
