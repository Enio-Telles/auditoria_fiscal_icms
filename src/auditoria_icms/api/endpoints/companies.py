...
async def list_companies(
    ...
    query = db.query(Empresa).join(
        UsuarioEmpresaAcesso,
        Empresa.id == UsuarioEmpresaAcesso.empresa_id
    ).filter(
...
@router.get("/{company_id}", response_model=CompanyResponse, summary="Buscar empresa por ID")
async def get_company(
    ...
    if not access:
        logger.warning(f"❌ Usuário {current_user.email} sem acesso à empresa {company_id}")
        raise HTTPException(
...
async def create_company(
    ...
    existing_company = db.query(Empresa).filter(Empresa.cnpj == company_data.cnpj).first()
    if existing_company:
...
async def update_company(
    ...
    if not access:
        logger.warning(f"❌ Usuário {current_user.email} sem permissão para editar empresa {company_id}")
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
@router.delete("/{company_id}", response_model=MessageResponse, summary="Desativar empresa")
async def deactivate_company(
    ...
    if not access:
        logger.warning(f"❌ Usuário {current_user.email} sem permissão admin para empresa {company_id}")
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
@router.post("/{company_id}/users/{user_id}/access", response_model=MessageResponse, summary="Conceder acesso à empresa")
async def grant_company_access(
    company_id: int,
    user_id: int,
    access_level: str = Query(..., regex="^(viewer|editor|admin)$", description="Nível de acesso"),
    db: Session = Depends(get_db_session),
    current_user: Usuario = Depends(get_current_user)
):
    """
    ...
    """
    logger.info(f"🔑 Concedendo acesso {access_level} ao usuário {user_id} para empresa {company_id}")
    
    # Verificar se usuário atual é admin da empresa
    admin_access = db.query(UsuarioEmpresaAcesso).filter(
        UsuarioEmpresaAcesso.usuario_id == current_user.id,
        UsuarioEmpresaAcesso.empresa_id == company_id,
        UsuarioEmpresaAcesso.nivel_acesso == "admin"
    ).first()
    ...
    existing_access = db.query(UsuarioEmpresaAcesso).filter(
