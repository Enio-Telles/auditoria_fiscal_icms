...
@router.get(
    ...
    summary="Listar resultados de classificação",
)
async def list_classification_results(
    ...
    """
    logger.info(
        f"📋 Listando resultados da empresa {empresa_id} - Usuário: {current_user.email}"
    )
    ...
    query = (
        ...
        .outerjoin(
            Classificacao, MercadoriaAClassificar.id == Classificacao.mercadoria_id
        )
        .filter(MercadoriaAClassificar.empresa_id == empresa_id)
        .options(joinedload(MercadoriaAClassificar.classificacao))
    )
    ...
    for mercadoria in mercadorias:
        classificacao = (
            mercadoria.classificacao[0] if mercadoria.classificacao else None
        )
        ...
        item = ProductClassificationResult(
            ...
            justificativa_ncm=(
                classificacao.justificativa_ncm if classificacao else None
            ),
            contexto_ncm=classificacao.contexto_ncm if classificacao else None,
            justificativa_cest=(
                classificacao.justificativa_cest if classificacao else None
            ),
...
@router.get(
    "/details/{mercadoria_id}",
    response_model=ClassificationDetailResponse,
    summary="Detalhes da classificação",
)
async def get_classification_details(
    ...
    if mercadoria.id_agregados:
        ...
        for similar in similar_mercadorias:
            similar_classificacao = (
                similar.classificacao[0] if similar.classificacao else None
            )
            ...
            similar_result = ProductClassificationResult(
                ...
                ncm_determinado=(
                    similar_classificacao.ncm_determinado
                    if similar_classificacao
                    else None
                ),
                cest_determinado=(
                    similar_classificacao.cest_determinado
                    if similar_classificacao
                    else None
                ),
                confianca_ncm=(
                    similar_classificacao.confianca_ncm
                    if similar_classificacao
                    else None
                ),
                confianca_cest=(
                    similar_classificacao.confianca_cest
                    if similar_classificacao
                    else None
                ),
                status=similar.status,
                justificativa_ncm=(
                    similar_classificacao.justificativa_ncm
                    if similar_classificacao
                    else None
                ),
                contexto_ncm=(
                    similar_classificacao.contexto_ncm
                    if similar_classificacao
                    else None
                ),
                justificativa_cest=(
                    similar_classificacao.justificativa_cest
                    if similar_classificacao
                    else None
                ),
                contexto_cest=(
                    similar_classificacao.contexto_cest
                    if similar_classificacao
                    else None
                ),
                processado_em=(
                    similar_classificacao.processado_em
                    if similar_classificacao
                    else None
                ),
            )
...
@router.put(
    "/{mercadoria_id}/status",
    response_model=MessageResponse,
    summary="Atualizar status da classificação",
)
async def update_classification_status(
    mercadoria_id: int,
    new_status: str = Query(
        ..., regex="^(PENDENTE|PROCESSANDO|CONCLUIDA|REVISAO_MANUAL|ERRO)$"
    ),
    db: Session = Depends(get_db_session),
    current_user: Usuario = Depends(get_current_user),
):
    ...
    """
    logger.info(
        f"📝 Atualizando status da mercadoria {mercadoria_id} para {new_status}"
    )
...
async def get_classification_stats(
    ...
    for status_code in status_list:
        count = (
            ...
            .filter(
                ...
                MercadoriaAClassificar.status == status_code,
            )
            .count()
        )
    ...
    if classificacoes:
        ...
        cest_confidences = [
            c.confianca_cest for c in classificacoes if c.confianca_cest
        ]

        if ncm_confidences:
            confidence_stats["avg_confidence_ncm"] = sum(ncm_confidences) / len(
                ncm_confidences
            )

        if cest_confidences:
            confidence_stats["avg_confidence_cest"] = sum(cest_confidences) / len(
                cest_confidences
            )
    ...
    return {
        ...
        "processing_rate": {
            ...
            "percentage_completed": (
                (stats_by_status["CONCLUIDA"] / total_produtos * 100)
                if total_produtos > 0
                else 0
            ),
        },
