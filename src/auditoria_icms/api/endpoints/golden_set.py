...
async def list_golden_set(
    ...
    empresa_filter: Optional[int] = Query(None, description="Filtrar por empresa fonte"),
...
@router.get("/{golden_id}", response_model=GoldenSetResponse, summary="Buscar entrada do Golden Set")
...
@router.post("/", response_model=GoldenSetResponse, summary="Criar entrada no Golden Set")
async def create_golden_set_entry(
    ...
    existing_entry = db.query(GoldenSet).filter(
        GoldenSet.descricao_produto == golden_data.descricao_produto
    ).first()
    
    if existing_entry:
        logger.warning(f"⚠️ Entrada similar já existe no Golden Set")
...
@router.put("/{golden_id}", response_model=GoldenSetResponse, summary="Atualizar entrada do Golden Set")
async def update_golden_set_entry(
    ...
    """
    logger.info(f"✏️ Atualizando entrada Golden Set {golden_id} - Por: {current_user.email}")
    ...
    try:
        ...
        raise APIError("Erro ao atualizar entrada do Golden Set", 500, {"error": str(e)})

@router.delete("/{golden_id}", response_model=MessageResponse, summary="Remover entrada do Golden Set")
async def delete_golden_set_entry(
    ...
    """
    logger.info(f"🗑️ Removendo entrada Golden Set {golden_id} - Por: {current_user.email}")
    ...
    try:
        ...
        return MessageResponse(
            message=f"Entrada do Golden Set removida com sucesso",
            success=True,
            data={
                "golden_id": golden_id,
                "descricao": descricao,
...
@router.post("/from-classification/{mercadoria_id}", response_model=GoldenSetResponse, summary="Criar Golden Set a partir de classificação")
async def create_from_classification(
    ...
    observacoes: Optional[str] = Query(None, description="Observações sobre a correção"),
    db: Session = Depends(get_db_session),
    current_user: Usuario = Depends(get_current_user)
):
    """
    ...
    from ...database.models import MercadoriaAClassificar, Classificacao, UsuarioEmpresaAcesso
    ...
    classificacao = db.query(Classificacao).filter(
        Classificacao.mercadoria_id == mercadoria_id
    ).first()
    ...
    golden_data = GoldenSetCreate(
        ...
        observacoes=observacoes or f"Correção humana da classificação automática. Original: NCM={classificacao.ncm_determinado if classificacao else 'N/A'}, CEST={classificacao.cest_determinado if classificacao else 'N/A'}"
    )
...
async def search_similar_in_golden_set(
    description: str = Query(..., min_length=5, description="Descrição para buscar similaridade"),
    threshold: float = Query(default=0.8, ge=0.1, le=1.0, description="Limiar de similaridade"),
...
async def get_golden_set_stats(
    ...
    user_contributions = db.query(
        GoldenSet.fonte_usuario,
        db.func.count(GoldenSet.id).label('count')
    ).group_by(GoldenSet.fonte_usuario).all()
    
    # Por empresa fonte
    company_contributions = db.query(
        GoldenSet.fonte_empresa_id,
        db.func.count(GoldenSet.id).label('count')
    ).group_by(GoldenSet.fonte_empresa_id).all()
    ...
    return {
        ...
        "by_user": [{"user": contrib[0], "count": contrib[1]} for contrib in user_contributions],
        "by_company": [{"company_id": contrib[0], "count": contrib[1]} for contrib in company_contributions],
        "quality_metrics": {
            "coverage_percentage": (with_cest / total_entries * 100) if total_entries > 0 else 0,
            "avg_description_length": db.query(db.func.avg(db.func.length(GoldenSet.descricao_produto))).scalar() or 0
        }
    }
