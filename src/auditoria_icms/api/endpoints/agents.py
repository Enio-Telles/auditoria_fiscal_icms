...
from ..schemas import AgentExecutionRequest, AgentExecutionResponse, MessageResponse
...
async def execute_agent_task(
    ...
    try:
            ...
            for produto in produtos:
                try:
                    ...
                    if agent_type == "enrichment":
                        ...
                        if result["success"]:
                            produto.descricao_enriquecida = result.get("enriched_description")
                            db.commit()
                        ...
                        if not ncm_to_use:
                            ...
                            ncm_to_use = ncm_result.get("ncm_code") if ncm_result["success"] else None
                    ...
                    agent_jobs_cache[job_id]["message"] = f"Processado {processed}/{total_produtos} ({progress:.1f}%)"
                    
                    logger.info(f"✅ Produto {produto.id} processado pelo agente {agent_type}")
        ...
        agent_jobs_cache[job_id]["message"] = f"Agente {agent_type} executado com sucesso!"
        
        successful = sum(1 for r in results if r["success"])
        logger.info(f"🎉 Agente {agent_type} concluído: {successful}/{total_produtos} sucessos")
...
@router.post("/execute", response_model=AgentExecutionResponse, summary="Executar agente específico")
async def execute_agent(
    ...
    """
    logger.info(f"🤖 Solicitação de execução do agente {agent_request.agent_type} - Empresa: {agent_request.empresa_id}")
...
async def get_agent_status(
    ...
    if job_id not in agent_jobs_cache:
        raise HTTPException(
            ...
            detail="Job de agente não encontrado"
        )
