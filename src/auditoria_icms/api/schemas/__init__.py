...
class UserCreate(BaseModel):
    ...
    identificacao: Optional[str] = Field(None, max_length=50, description="Identificação interna")
...
class CompanyCreate(BaseModel):
    ...
    atividades: List[str] = Field(..., min_items=1, description="Lista de atividades da empresa")
...
class DataImportRequest(BaseModel):
    ...
    database_config: DatabaseConnection = Field(..., description="Configuração do banco")
    table_name: str = Field(default="produto", description="Nome da tabela a importar")
    limit: Optional[int] = Field(None, gt=0, description="Limite de registros (opcional)")
    filters: Optional[Dict[str, Any]] = Field(None, description="Filtros SQL adicionais")
...
class DataImportResponse(BaseModel):
    ...
    total_records: Optional[int] = Field(None, description="Total de registros a importar")
...
class BatchClassificationRequest(BaseModel):
    ...
    empresa_id: int = Field(..., description="ID da empresa")
    limit: Optional[int] = Field(None, gt=0, description="Limite de produtos a classificar")
...
class ClassificationJobStatus(BaseModel):
    ...
    estimated_remaining: Optional[int] = Field(None, description="Tempo restante estimado (segundos)")
    current_item: Optional[str] = Field(None, description="Item sendo processado atualmente")
...
class GoldenSetCreate(BaseModel):
    ...
    descricao_produto: str = Field(..., min_length=5, description="Descrição do produto")
    descricao_enriquecida: Optional[str] = Field(None, description="Descrição enriquecida")
    gtin: Optional[str] = Field(None, description="GTIN/código de barras")
    ncm_correto: str = Field(..., min_length=8, max_length=10, description="NCM correto")
    cest_correto: Optional[str] = Field(None, min_length=7, max_length=7, description="CEST correto")
...
class AgentExecutionRequest(BaseModel):
    ...
    empresa_id: int
    produto_ids: Optional[List[int]] = Field(None, description="IDs específicos para processar")
    parameters: Optional[Dict[str, Any]] = Field(None, description="Parâmetros específicos do agente")
