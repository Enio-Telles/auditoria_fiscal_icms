...
class ReconciliationAgent(BaseAgent):
    ...
    async def process(
        ...
        golden_set_validation = await self._validate_against_golden_set(
            descricao, conflict_resolution['ncm_final'], conflict_resolution['cest_final']
        )
        
        # 5. Análise de confiança final
        final_confidence = await self._calculate_final_confidence(
            ncm_result, cest_result, cross_validation_result, 
            consistency_analysis, golden_set_validation
        )
        
        # 6. Gerar justificativas detalhadas
        detailed_justification = await self._generate_detailed_justification(
            ncm_result, cest_result, cross_validation_result,
            consistency_analysis, conflict_resolution, golden_set_validation
        )
    ...
    def validate_input(self, input_data: Dict[str, Any]) -> bool:
        ...
        return (
            'ncm_result' in input_data and 
            'cest_result' in input_data and
            ('descricao_produto' in input_data or 'descricao_enriquecida' in input_data)
        )
    ...
    async def _cross_validate_ncm_cest(
        ...
        try:
            ...
            if self.retrieval_tools:
                cest_rules = await self.retrieval_tools.get_cest_ncm_rules(cest_classificado)
    ...
    async def _analyze_consistency(
        ...
        if len(ncm_alternatives) > 2:
            high_conf_alternatives = [
                alt for alt in ncm_alternatives 
        ...
        if len(cest_alternatives) > 2:
            high_conf_cest_alternatives = [
                alt for alt in cest_alternatives 
            ...
            analysis['cest_consistency']['consistency_score'] = min(cest_confidence, 0.9)
    ...
    async def _resolve_incompatibility_conflict(
        ...
        if ncm_conf > cest_conf:
            ...
            if self.retrieval_tools:
                try:
                    compatible_cests = await self.retrieval_tools.search_cest_by_ncm(ncm_final)
                    if compatible_cests:
    ...
    async def _resolve_high_severity_conflicts(
        ...
        if ncm_alternatives:
            ...
            if best_ncm_alt.get('confidence', 0) > ncm_result.get('confidence_score', 0):
                resolution.update({
        ...
        if cest_alternatives:
            ...
            if best_cest_alt.get('confidence', 0) > cest_result.get('confidence_score', 0):
                resolution.update({
    ...
    async def _validate_against_golden_set(
        ...
        try:
            ...
            for match in golden_matches:
                if (match.get('ncm') == ncm_final and 
                    match.get('cest') == cest_final):
                    return {
                        "match_found": True,
