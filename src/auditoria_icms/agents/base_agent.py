...
class BaseAgent(ABC):
    ...
    async def explain_decision(self, decision_id: str) -> str:
        ...
        """
        decision = next(
            (d for d in self.decisions_history if d.decision_id == decision_id), None
        )
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        avg_confidence = (
            sum(d.confidence_score for d in self.decisions_history) / total_decisions
        )
        avg_processing_time = (
            sum(d.processing_time_ms for d in self.decisions_history) / total_decisions
        )
