...
class EnrichmentAgent:
    ...
    def _load_enrichment_rules(self) -> Dict[str, Any]:
        ...
        return {
            "required_fields": [
                "codigo_produto", "descricao", "ncm", "empresa_id"
            ],
            "optional_fields": [
                "cest", "unidade", "preco", "categoria", "atividade_empresa"
            ],
            "validation_rules": {
                "ncm": {
                    "length": 8,
    ...
    def enrich_product_data(self, product_data: Dict[str, Any], empresa_info: Dict[str, Any] = None) -> EnrichmentResult:
        ...
        try:
            ...
            for step in self.enrichment_rules["enrichment_priority"]:
                step_result = self._execute_enrichment_step(step, enriched_data, empresa_info)
    ...
    def enrich_batch_data(self, products_data: List[Dict[str, Any]], empresa_info: Dict[str, Any] = None) -> List[EnrichmentResult]:
    ...
    def _execute_enrichment_step(self, step: str, data: Dict[str, Any], empresa_info: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def _validate_ncm_step(self, data: Dict[str, Any], empresa_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _determine_missing_ncm_step(self, data: Dict[str, Any], empresa_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if result["success"] and result["ncm_determinado"]:
            ...
            warnings.append(f"NCM não determinado: {result.get('reason', 'Razão desconhecida')}")
    ...
    def _validate_cest_step(self, data: Dict[str, Any], empresa_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _determine_missing_cest_step(self, data: Dict[str, Any], empresa_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if result["success"]:
            ...
            warnings.append(f"CEST não determinado: {result.get('reason', 'Razão desconhecida')}")
    ...
    def _enrich_metadata_step(self, data: Dict[str, Any], empresa_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if empresa_info:
            ...
            enriched_data["empresa_regime_tributario"] = empresa_info.get("regime_tributario")
    ...
    def _validate_business_rules_step(self, data: Dict[str, Any], empresa_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if cest and empresa_info:
            ...
            if "simples" in regime and cest:
                warnings.append("Empresas do Simples Nacional podem não precisar de CEST")
    ...
    def _calculate_completeness_score(self, data: Dict[str, Any]) -> float:
        ...
        # Campos obrigatórios
        required_present = sum(1 for field in self.enrichment_rules["required_fields"] 
                             if data.get(field))
        required_score = required_present / len(self.enrichment_rules["required_fields"])
        
        # Campos opcionais
        optional_present = sum(1 for field in self.enrichment_rules["optional_fields"] 
                             if data.get(field))
        optional_score = optional_present / len(self.enrichment_rules["optional_fields"])
...
class ReconciliationAgent:
    ...
    def _load_reconciliation_rules(self) -> Dict[str, Any]:
        ...
        return {
            "conflict_resolution": {
                "ncm": {
                    "priority": ["manual", "agent_high_confidence", "database", "default"],
    ...
    def reconcile_product_data(self, sources: List[Dict[str, Any]], source_names: List[str] = None) -> ReconciliationResult:
        ...
        try:
            ...
            for field in all_fields:
                field_values = [(source.get(field), name) for source, name in zip(sources, source_names) 
                ...
                if len(field_values) == 1:
                    ...
                    # Resolver conflito
                    resolution = self._resolve_field_conflict(field, field_values, conflicts)
            ...
            success = overall_confidence > 0.5 and not any(c["severity"] == "critical" for c in conflicts)
    ...
    def _identify_conflicts(self, sources: List[Dict[str, Any]], source_names: List[str]) -> List[Dict[str, Any]]:
    ...
    def _resolve_field_conflict(self, field: str, field_values: List[Tuple[Any, str]], conflicts: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for strategy in priority:
            if strategy == "manual":
                ...
                for value, source in field_values:
                    if isinstance(value, dict) and value.get("confidence", 0) > field_rules.get("confidence_threshold", 0.8):
                        return {
                            "field": field,
                            "value": value,
                            "source": source,
    ...
    def generate_reconciliation_report(self, results: List[ReconciliationResult]) -> Dict[str, Any]:
        ...
        return {
            "summary": {
                ...
                "success_rate": successful_reconciliations / total_products if total_products > 0 else 0,
                "manual_reviews_required": manual_reviews_required,
                "manual_review_rate": manual_reviews_required / total_products if total_products > 0 else 0
            },
