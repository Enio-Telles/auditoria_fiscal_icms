...
class NCMAgent(BaseAgent):
    ...
    def __init__(self, llm, config: Dict[str, Any], logger=None):
        ...
        self.hierarchy_levels = config.get(
    ...
    async def _confirm_existing_ncm(
        self, descricao: str, ncm_atual: str
    ) -> Dict[str, Any]:
    ...
    async def _classify_position(
        self, descricao: str, capitulo: str
    ...
    async def _classify_subposition(
        self, descricao: str, posicao: str
    ) -> Optional[Dict[str, Any]]:
        """Classifica em uma subposição específica."""
        if not self.retrieval_tools:
            return None

        subpositions = await self.retrieval_tools.search_subpositions(
            descricao, posicao
        )
    ...
    async def _classify_item(
        self, descricao: str, subposicao: str
    ) -> Optional[Dict[str, Any]]:
    ...
    async def _analyze_description_ncm_compatibility(
        self, descricao: str, ncm: str, ncm_info: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        ncm_desc_keywords = self._extract_keywords(
            ncm_info.get("descricao", "").lower()
        )
        ...
        has_exclusions = any(
            exclusion.lower() in descricao.lower() for exclusion in exclusions
        )
            ...
            candidates.append(
                {
                    ...
                    "source": "hierarchical",
                    "justificativa": (
                        "Classificação hierárquica - nível: "
                        f"{hierarchical_result.get('level', 'completo')}"
                    ),
                }
            )
