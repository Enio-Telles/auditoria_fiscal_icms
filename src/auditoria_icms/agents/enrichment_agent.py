...
class EnrichmentAgent(BaseAgent):
    ...
    def __init__(self, llm, config: Dict[str, Any], logger=None):
        ...
        self.enhancement_strategies = config.get('enhancement_strategies', [
            'semantic', 'regulatory', 'contextual'
        ])
    ...
    async def process(
        ...
        for strategy in self.enhancement_strategies:
            try:
                if strategy == 'semantic':
                    result = await self._semantic_enhancement(descricao_original, analysis)
                elif strategy == 'regulatory':
                    result = await self._regulatory_enhancement(descricao_original, ncm_sugerido)
        ...
        technical_features = await self._extract_technical_features(enriched_description)
    ...
    async def _analyze_description(self, description: str) -> Dict[str, Any]:
        ...
        # Verificar materiais
        materials = ["aço", "plástico", "alumínio", "ferro", "cobre", "madeira", "tecido", 
                    "vidro", "cerâmica", "borracha", "silicone"]
        analysis["has_materials"] = any(material in description_lower for material in materials)
        
        # Avaliar qualidade da estrutura
        if analysis["word_count"] > 10 and (analysis["has_technical_terms"] or analysis["has_materials"]):
