...
from ..database.models import ProdutoEmpresa, AuditoriaAgentesLog, StatusProcessamento
...
class BatchProcessingResult:
    ...
    def to_dict(self):
        ...
        return {
            ...
            'resultados_individuais': [r.to_dict() for r in self.resultados_individuais]
        }
...
class ManagerAgent(BaseAgent):
    ...
    def __init__(self, 
        ...
        self.config_processamento = {
            ...
            'auto_approve_threshold': config.get('auto_approve_threshold', 0.9) if config else 0.9,
    ...
    def processar_lote(self, 
        ...
        try:
            ...
            for i, produto in enumerate(produtos):
                try:
                    logger.info(f"Processando produto {i+1}/{len(produtos)}: {produto.produto_id}")
                    ...
                    logger.error(f"Erro ao processar produto {produto.produto_id}: {str(e)}")
            ...
            logger.info(f"Processamento concluído. Sucesso: {produtos_com_sucesso}, "
    ...
    def _processar_produto_individual(self, produto: ProdutoEmpresa) -> ProcessingResult:
        """
            ...
            while estado_atual not in ['CONCLUIDO', 'ERRO', 'REVISAO_MANUAL']:
                ...
                if estado_atual == 'PENDENTE':
                    ...
                    if resultado_enrich['status'] == 'sucesso':
                        produto_processado.descricao_enriquecida = resultado_enrich['resultado'].get('descricao_enriquecida', '')
                    ...
                    acao = 'confirmar_ncm' if hasattr(produto_processado, 'ncm') and produto_processado.ncm else 'determinar_ncm'
                    ...
                    if resultado_ncm['status'] == 'sucesso':
                        produto_processado.ncm_sugerido = resultado_ncm['resultado'].get('ncm', '')
                        produto_processado.confianca_ncm = resultado_ncm['resultado'].get('confianca', 0.0)
                        produto_processado.justificativa_ncm = resultado_ncm['resultado'].get('justificativa', '')
                        
                        # Verifica se precisa de revisão manual
                        if produto_processado.confianca_ncm < self.config_processamento['confianca_minima']:
                    ...
                    acao = 'confirmar_cest' if hasattr(produto_processado, 'cest') and produto_processado.cest else 'determinar_cest'
                    ...
                    if resultado_cest['status'] == 'sucesso':
                        produto_processado.cest_sugerido = resultado_cest['resultado'].get('cest', '')
                        produto_processado.confianca_cest = resultado_cest['resultado'].get('confianca', 0.0)
                        produto_processado.justificativa_cest = resultado_cest['resultado'].get('justificativa', '')
                    ...
                    if resultado_reconcil['status'] == 'sucesso':
                        ...
                        if confianca_media >= self.config_processamento['auto_approve_threshold']:
    ...
    def _executar_agente(self, 
                        agente: BaseAgent, 
                        acao: str, 
                        produto: ProdutoEmpresa,
                        logs_agentes: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            dados_entrada = {
                ...
                'descricao_enriquecida': getattr(produto, 'descricao_enriquecida', None),
        ...
        for resultado in resultados:
            if resultado.produto_atualizado:
                produto_dict = {
                    ...
                    'descricao_enriquecida': getattr(resultado.produto_atualizado, 'descricao_enriquecida', None),
                    'ncm_sugerido': getattr(resultado.produto_atualizado, 'ncm_sugerido', None),
                    'cest_sugerido': getattr(resultado.produto_atualizado, 'cest_sugerido', None),
                    'status_processamento': getattr(resultado.produto_atualizado, 'status_processamento', None),
                    'confianca_ncm': getattr(resultado.produto_atualizado, 'confianca_ncm', None),
                    'confianca_cest': getattr(resultado.produto_atualizado, 'confianca_cest', None),
                    'justificativa_ncm': getattr(resultado.produto_atualizado, 'justificativa_ncm', None),
                    'justificativa_cest': getattr(resultado.produto_atualizado, 'justificativa_cest', None),
                    'revisao_manual': getattr(resultado.produto_atualizado, 'revisao_manual', False)
                }
                produtos_para_atualizar.append(produto_dict)
        
        if produtos_para_atualizar:
            sucesso = self.data_ingestion.update_produtos_processados(produtos_para_atualizar)
            
            if sucesso:
                logger.info(f"Atualizados {len(produtos_para_atualizar)} produtos no banco da empresa")
            else:
                logger.error(f"Erro ao atualizar produtos no banco da empresa")
    
    def _registrar_status_processamento(self, task_id: str, total_produtos: int, status: str):
        """Registra início do processamento"""
        logger.info(f"Iniciando processamento {task_id} - {total_produtos} produtos - Status: {status}")
    
    def _atualizar_status_processamento(self, task_id: str, produtos_processados: int, status: str):
        """Atualiza status do processamento"""
        logger.info(f"Processamento {task_id} - {produtos_processados} produtos processados - Status: {status}")
...
def create_manager_agent(empresa_id: int, 
                        db_config: Dict[str, Any],
                        processing_config: Optional[Dict[str, Any]] = None) -> ManagerAgent:
