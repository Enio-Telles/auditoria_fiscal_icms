...
from .base_agent_simple import BaseAgent, EnrichmentAgentBase, NCMAgentBase, CESTAgentBase, ReconciliationAgentBase
...
class ManagerAgent(BaseAgent):
    ...
    def processar_lote(self, produtos: List[Any], batch_size: Optional[int] = None) -> BatchProcessingResult:
        """
        ...
        try:
            ...
            for i, produto in enumerate(produtos):
                try:
                    logger.info(f"Processando produto {i+1}/{len(produtos)}: {getattr(produto, 'produto_id', f'produto_{i}')}")
                    ...
                    logger.error(f"Erro ao processar produto {getattr(produto, 'produto_id', f'produto_{i}')}: {str(e)}")
