...
from .base_agent import BaseAgent, AuditTrail
...
class ManagerAgent(BaseAgent):
    ...
    async def process(
        ...
        workflow_result = await self._execute_workflow(
            workflow_type, input_data, context, audit_trail
        )
        ...
        for step in workflow_steps:
            ...
            try:
                ...
                if step == "enrichment" and step_result.get("enriched_description"):
                    current_data["descricao_enriquecida"] = step_result[
    ...
    async def _make_final_decision(
        self, workflow_result: Dict[str, Any], audit_trail: AuditTrail
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "sources": list(
                set(ncm_result.get("sources", []) + cest_result.get("sources", []))
            ),
            "method": "individual_consensus",
        }

    def _requires_human_review(
        self, final_decision: Dict[str, Any], audit_trail: AuditTrail
    ) -> bool:
        ...
        ncm_classifications = [
            ...
            if d.agent_name.endswith("_classifier")
            and d.output_data.get("ncm_classificado")
        ]
        ...
        if business_rules.get("require_human_validation", {}).get(
    ...
    def _generate_reasoning(
        self, workflow_result: Dict[str, Any], final_decision: Dict[str, Any]
    ) -> str:
    ...
    def get_workflow_statistics(self) -> Dict[str, Any]:
        ...
        workflow_types = [
            d.output_data.get("workflow_type", "unknown") for d in decisions
        ]

        human_reviews = sum(
            1 for d in decisions if d.output_data.get("requires_human_review", False)
        )
        ...
        return {
            ...
            "average_processing_time_ms": sum(d.processing_time_ms for d in decisions)
            / len(decisions),
        }
