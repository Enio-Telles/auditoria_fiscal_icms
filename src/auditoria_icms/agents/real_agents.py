...
class NCMAgent:
    ...
    def _load_ncm_data(self):
        ...
        try:
            ...
            if (data_path / "descricoes_ncm.json").exists():
                with open(
                    data_path / "descricoes_ncm.json", "r", encoding="utf-8"
                ) as f:
    ...
    def validate_ncm(
        self, ncm_code: str, description: str, empresa_atividade: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "justificativa": (
                    "NCM validado com confiança "
                    f"{compatibility_score:.2%}. "
                    f"{hierarchy_info['explanation']}"
                ),
            }
    ...
    def determine_ncm(
        self, description: str, empresa_atividade: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "justificativa": (
                    f"NCM {best_candidate['ncm']} determinado com "
                    f"{best_candidate['score']:.2%} de confiança"
                ),
    ...
    def _find_ncm_candidates(
        ...
        for item in self.ncm_data:
            ...
            if score > 0.3:  # Threshold mínimo
                candidates.append(
                    {"ncm": item.get("Código"), "score": score, "info": item}
                )

        return sorted(candidates, key=lambda x: x["score"], reverse=True)[:10]

    def _calculate_description_compatibility(
        self, product_desc: str, ncm_desc: str, empresa_atividade: str = None
    ) -> float:
...
class CESTAgent:
    ...
    def _load_cest_data(self):
        ...
        try:
            ...
            if (data_path / "conv_142_formatado.json").exists():
                with open(
                    data_path / "conv_142_formatado.json", "r", encoding="utf-8"
                ) as f:
                    self.cest_data = json.load(f)
                self.logger.info(
                    f"Carregados {len(self.cest_data)} itens CEST do Convênio 142"
                )
    ...
    def validate_cest(
        ...
        try:
            ...
            ncm_compatibility = self._check_ncm_cest_compatibility(
                ncm_code, cest_code, cest_info
            )

            # Verificar segmento da empresa
            segment_compatibility = self._check_segment_compatibility(
                empresa_atividade, cest_info
            )

            # Score final
            final_score = min(
                ncm_compatibility["score"] * segment_compatibility["score"], 1.0
            )
            ...
            return {
                ...
                "justificativa": (
                    "CEST validado com " f"{final_score:.2%} de confiança"
                ),
            }
    ...
    def determine_cest(
        ...
        try:
            ...
            candidates = self._find_cest_candidates_by_ncm(
                ncm_code, description, empresa_atividade
            )
            ...
            return {
                ...
                "justificativa": (
                    f"CEST {best_candidate['cest']} determinado com "
                    f"{best_candidate['score']:.2%} de confiança"
                ),
                "candidates": candidates[:3],
            }
    ...
    def _find_cest_candidates_by_ncm(
        self, ncm_code: str, description: str, empresa_atividade: str = None
    ) -> List[Dict]:
        ...
        if self.cest_data:
            for item in self.cest_data:
                ...
                ncm_match_score = self._calculate_ncm_match_score(
                    ncm_code, item.get("ncm", "")
                )

                if ncm_match_score > 0:
                    # Score baseado na descrição
                    desc_score = self._calculate_description_score(
                        description, item.get("descricao_oficial_cest", "")
                    )

                    # Score baseado no segmento da empresa
                    segment_score = self._calculate_segment_score(
                        empresa_atividade, item.get("Segmento", "")
                    )

                    # Score final
                    final_score = (
                        ncm_match_score * 0.5 + desc_score * 0.3 + segment_score * 0.2
                    )
    ...
    def _check_ncm_cest_compatibility(
        self, ncm_code: str, cest_code: str, cest_info: Dict
    ) -> Dict[str, Any]:
    ...
    def _check_segment_compatibility(
        self, empresa_atividade: str, cest_info: Dict
    ) -> Dict[str, Any]:
        ...
        if not empresa_atividade:
            return {
                "score": 1.0,
                "compatible": True,
                "explanation": "Atividade não informada",
            }
            ...
            if activity in atividade_lower:
                if any(seg in str(segmento) for seg in segments):
                    return {
                        "score": 1.0,
                        "compatible": True,
                        "explanation": f"Segmento {segmento} compatível com {activity}",
                    }
                else:
                    return {
                        "score": 0.3,
                        "compatible": False,
                        "explanation": f"Segmento {segmento} pode não ser compatível com {activity}",
                    }

        return {
            "score": 0.8,
            "compatible": True,
            "explanation": "Compatibilidade de segmento não verificada",
        }
    ...
    def _calculate_segment_score(self, empresa_atividade: str, segmento: str) -> float:
        ...
        return self._check_segment_compatibility(
            empresa_atividade, {"Segmento": segmento}
        )["score"]
