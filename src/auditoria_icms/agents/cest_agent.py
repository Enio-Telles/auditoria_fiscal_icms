...
class CESTAgent(BaseAgent):
    ...
    async def process(
        ...
        ncm_classificado = input_data.get("ncm_final") or input_data.get(
            "ncm_classificado"
        )
    ...
    def validate_input(self, input_data: Dict[str, Any]) -> bool:
        ...
        descricao = input_data.get("descricao_produto") or input_data.get(
        ...
        return (
            ...
            and len(descricao.strip()) >= 3
            and ncm
            and isinstance(ncm, str)
            and len(ncm) >= 6
        )

    async def _confirm_existing_cest(
        self, descricao: str, ncm: str, cest_atual: str, estado: str
    ...
    async def _search_by_exact_ncm(
        self, ncm: str, estado: str
    ) -> Optional[Dict[str, Any]]:
    ...
    async def _search_by_ncm_pattern(
        self, ncm: str, estado: str
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for length in [8, 6, 4, 2]:
                pattern = ncm[:length]
                matches = await self.retrieval_tools.search_cest_by_pattern(
                    pattern, estado
                )
    ...
    async def _semantic_search_cest(
        self, descricao: str, estado: str
    ) -> Optional[Dict[str, Any]]:
    ...
    async def _analyze_substituicao_tributaria(
        self, descricao: str, ncm: str, estado: str
    ) -> Dict[str, Any]:
        ...
        for category, info in st_categories.items():
            ...
            ncm_match = any(ncm.startswith(pattern) for pattern in info["ncm_patterns"])
        ...
        return {
            ...
            "reasoning": (
                f"Categoria: {category_matched}"
                if category_matched
                else "Não identificado como sujeito à ST"
            ),
        }

    async def _check_ncm_in_cest_rules(
        self, ncm: str, cest: str, estado: str
    ) -> Dict[str, Any]:
    ...
    async def _consolidate_cest_results(
            ...
            candidates.append(
                {
                    ...
                    "source": "exact_ncm",
                    # Linha longa encurtada
                    "justificativa": (
                        "NCM corresponde exatamente às regras do CEST "
                        f"{exact_match_result['cest']}"
                    ),
            ...
            candidates.append(
                {
                    ...
                    "justificativa": (
                        "NCM corresponde ao padrão "
                        f"{pattern_result['pattern_matched']} do CEST "
                        f"{pattern_result['cest']}"
                    ),
            ...
            candidates.append(
                {
                    ...
                    "source": "semantic",
                    "justificativa": "Baseado em similaridade semântica com produtos conhecidos",
                    "segmento": None,
                }
            )
        ...
        if not candidates:
            ...
            if st_analysis.get("st_aplicavel"):
                return {
                    ...
                    "confidence": 0.0,
                    "justificativa": (
                        "Produto pode estar sujeito à substituição tributária "
                        f"(categoria: {st_analysis.get('categoria')}) sem CEST específico"
                    ),
                    "st_aplicavel": True,
        ...
        alternatives = [
            {"cest": c["cest"], "confidence": c["confidence"], "source": c["source"]}
            for c in candidates
            if c != best_candidate
        ]
