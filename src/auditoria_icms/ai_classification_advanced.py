...
class LLMManager:
    ...
    def _track_usage(self, provider: str, result: Dict):
        ...
        if provider not in self.cost_tracker:
            self.cost_tracker[provider] = {"requests": 0, "tokens": 0, "cost": 0.0}
        ...
        try:
            response = await self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Você é um especialista em classificação fiscal NCM/CEST.",
                    },
...
class HuggingFaceProvider(BaseProvider):
    ...
    async def generate(self, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # Gerar resposta (torch removido, geração direta)
...
class AdvancedNCMCESTClassifier:
    ...
    def load_config(self, config_path: str) -> Dict[str, Any]:
        ...
        default_config = {
            "llm": {
                ...
                "huggingface": {"enabled": False, "model": "microsoft/DialoGPT-medium"},
            },
            "classification": {
            ...
            "caching": {"enabled": True, "ttl_hours": 24, "max_size": 10000},
        }
        ...
        try:
            ...
            logger.warning(
                f"Erro ao carregar config {config_path}: {e}. Usando padrão."
            )
    ...
    def _select_classification_strategy(
        self, request: ClassificationRequest
    ...
    async def _classify_direct(
        ...
        response = await self.llm_manager.generate_response(
            prompt, provider="auto", complexity="low"
        )

        return self._parse_llm_response(request, response, "direct")

    async def _classify_rag(
        self, request: ClassificationRequest
    ) -> ClassificationResult:
        ...
        response = await self.llm_manager.generate_response(
            prompt, provider="auto", complexity="medium"
        )
        ...
        for model, task in tasks:
            try:
                ...
                parsed = self._parse_llm_response(
                    request, response, f"ensemble-{model}"
                )
    ...
    async def _classify_hierarchical(
        ...
        posicao_prompt = self._build_hierarchical_prompt(
            request, "posicao", capitulo_response
        )
...
"""
    ...
    def _build_rag_prompt(
        self, request: ClassificationRequest, ncm_candidates: List[Dict]
    ) -> str:
...
"""
    ...
    def _build_hierarchical_prompt(
        self, request: ClassificationRequest, level: str, previous_response: Dict = None
    ) -> str:
...
"""
    ...
    def _parse_llm_response(
        self, request: ClassificationRequest, response: Dict, strategy: str
    ) -> ClassificationResult:
        ...
        try:
            ...
            if json_match:
                ...
                return ClassificationResult(
                    ...
                    cest_confianca=(
                        float(data.get("cest_confianca", 0.0))
                        if data.get("cest_confianca")
                        else None
                    ),
    ...
    def _consolidate_ensemble_results(
        self, request: ClassificationRequest, results: List[ClassificationResult]
    ) -> ClassificationResult:
    ...
    async def _validate_and_enhance_result(
        ...
        if result.ncm_confianca < min_confidence:
            ...
            result.validation_flags.append(
                f"Confiança abaixo do limiar ({min_confidence})"
            )

        # Verificar compatibilidade NCM/CEST
        if result.cest_sugerido and result.ncm_sugerido:
            cest_matches = self.knowledge_base.search_cest_by_ncm(
                result.ncm_sugerido, request.estado
            )
    ...
    def _create_error_result(
        self, request: ClassificationRequest, error: str
    ) -> ClassificationResult:
