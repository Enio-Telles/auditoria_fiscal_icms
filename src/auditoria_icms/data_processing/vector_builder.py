...
class VectorBuilder:
    ...
    def _initialize_encoder(self):
        ...
        try:
            ...
            logger.info(f"Modelo carregado. Dimensão dos vetores: {encoder.get_sentence_embedding_dimension()}")
            return encoder
        except ImportError:
    ...
    def create_document_chunks(self, text: str, chunk_size: int = 512, overlap: int = 50) -> List[str]:
    ...
    def process_ncm_descriptions(self) -> List[Dict]:
        ...
        for item in ncm_data:
            ...
            enhanced_text = f"NCM {codigo}: {descricao}. Capítulo {capitulo}, Posição {posicao}, Subposição {subposicao}."
    ...
    def process_cest_rules(self) -> List[Dict]:
        ...
        cest_data = [
            {"cest": "28.064.00", "descricao": "Artigos de vestuário infantil", "segmento": "Vestuário"},
            {"cest": "17.004.00", "descricao": "Medicamentos genéricos", "segmento": "Medicamentos"},
            {"cest": "21.001.00", "descricao": "Aparelhos telefônicos", "segmento": "Telecomunicações"},
        ]
        ...
        for item in cest_data:
            ...
            # Cria texto enriquecido
            enhanced_text = f"CEST {cest_codigo}: {descricao}. Segmento: {segmento}. Aplicável para substituição tributária."
    ...
    def build_vector_database(self) -> Dict:
        ...
        logger.info(f"Base vetorial construída:")
    ...
    def save_vector_database(self, vector_db: Dict, faiss_index: Any = None):
        ...
        logger.info(f"Base vetorial salva:")
    ...
    def test_vector_search(self, vector_db: Dict, embeddings: np.ndarray):
        ...
        try:
            ...
            for i, idx in enumerate(top_indices):
                ...
                logger.info(f"{i+1}. [{doc['doc_type']}] {doc['content'][:100]}... (score: {score:.3f})")
