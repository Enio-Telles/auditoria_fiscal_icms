...
class DataExtractor:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            logger.info(f"✅ Conexão estabelecida com {self.db_config.db_type} em {self.db_config.host}")
            return True
    ...
    def get_table_info(self, table_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Contar registros
            count_query = f"SELECT COUNT(*) as total FROM {self.db_config.schema}.{table_name}"
    ...
    def extract_data(self, config: ExtractionConfig) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            logger.info(f"🔄 Iniciando extração de dados...")
            ...
            if config.chunk_size and config.chunk_size > 0:
                ...
                chunks = []
                for chunk in pd.read_sql_query(query, self.engine, chunksize=config.chunk_size):
                    chunks.append(chunk)
    ...
    def _clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if 'descricao_produto' in df.columns:
                ...
                # Limpar descrições
                df['descricao_produto'] = df['descricao_produto'].str.strip().str.upper()
            ...
            if 'codigo_barra' in df.columns:
                ...
                df['codigo_barra'] = df['codigo_barra'].astype(str).str.replace(r'\D', '', regex=True)
...
def create_extractor_from_env() -> DataExtractor:
    ...
    for attr in required_configs:
        if not getattr(config, attr):
            raise ValueError(f"Configuração {attr.upper()} não encontrada no arquivo .env")
