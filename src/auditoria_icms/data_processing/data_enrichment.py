...
class DataEnrichment:
    ...
    def _load_category_mappings(self) -> Dict[str, List[str]]:
        ...
        return {
            'medicamentos': ['medicamento', 'remedio', 'farmaco', 'comprimido', 'capsula', 'xarope', 'antibiotico'],
            'alimentos': ['alimento', 'comida', 'bebida', 'leite', 'queijo', 'pao', 'biscoito', 'chocolate'],
            'cosmeticos': ['cosmetico', 'perfume', 'shampoo', 'sabonete', 'creme', 'maquiagem', 'batom'],
            'eletronicos': ['eletronico', 'celular', 'smartphone', 'tablet', 'computador', 'televisao', 'radio'],
            'vestuario': ['roupa', 'camisa', 'calca', 'vestido', 'sapato', 'tenis', 'bolsa', 'carteira'],
            'casa_jardim': ['movel', 'mesa', 'cadeira', 'sofa', 'cama', 'geladeira', 'fogao', 'microondas'],
            'automotivo': ['carro', 'automovel', 'peca', 'pneu', 'oleo', 'filtro', 'bateria', 'motor'],
            'esporte': ['esporte', 'bola', 'tenis', 'bicicleta', 'equipamento', 'academia', 'fitness'],
            'livros_midias': ['livro', 'revista', 'jornal', 'cd', 'dvd', 'bluray', 'game', 'jogo'],
            'brinquedos': ['brinquedo', 'boneca', 'carrinho', 'jogo', 'infantil', 'crianca'],
        }
    ...
    def extract_brand_from_description(self, description: str) -> Optional[str]:
        ...
        for pattern in brand_patterns:
            ...
            if matches:
                ...
                common_words = {'UNIDADE', 'PACOTE', 'CAIXA', 'FRASCO', 'TUBO', 'ML', 'MG', 'KG', 'G'}
                if potential_brand.upper() not in common_words and len(potential_brand) > 2:
    ...
    def extract_technical_specs(self, description: str) -> Dict[str, Any]:
        ...
        for pattern in volume_patterns:
            ...
            if match:
                ...
                specs['unidade_volume'] = pattern.split('?')[0].split('|')[-1].strip(')')
                break
    ...
    def enrich_products_dataset(self, products_df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"Enriquecimento concluído: {len(enriched_df)} produtos processados")
    ...
    def save_enriched_data(self, enriched_df: pd.DataFrame, indices: Dict, output_prefix: str = "enriched"):
        """Salva dados enriquecidos e índices."""
        os.makedirs(self.processed_dir, exist_ok=True)
        
        # Salva DataFrame enriquecido
        products_file = os.path.join(self.processed_dir, f"{output_prefix}_products.json")
        enriched_df.to_json(products_file, orient='records', indent=2, force_ascii=False)
    ...
    def generate_enrichment_stats(self, enriched_df: pd.DataFrame) -> Dict:
        ...
        stats = {
            ...
            'produtos_com_marca': len(enriched_df[enriched_df['marca_extraida'].notna()]),
            'produtos_categorizados': len(enriched_df[enriched_df['categoria_inferida'].notna()]),
            'distribuicao_categorias': enriched_df['categoria_inferida'].value_counts().to_dict(),
            'distribuicao_marcas': enriched_df['marca_extraida'].value_counts().head(20).to_dict(),
            'ncms_unicos': enriched_df['ncm'].nunique() if 'ncm' in enriched_df.columns else 0,
    ...
    def run_enrichment_test(self):
        ...
        if os.path.exists(products_file):
            ...
            stats = self.generate_enrichment_stats(enriched_df)
            logger.info(f"Estatísticas do enriquecimento:")
            ...
            test_products = [
                {'descricao': 'DIPIRONA SODICA 500MG COMPRIMIDO MEDLEY', 'gtin': '1234567890123', 'ncm': '30049069'},
                {'descricao': 'SMARTPHONE SAMSUNG GALAXY A54 128GB', 'gtin': '1234567890124', 'ncm': '85171211'},
                {'descricao': 'COCA-COLA 350ML LATA', 'gtin': '1234567890125', 'ncm': '22021000'},
            ]
