...
class DatabaseImporter:
    ...
    async def import_products_from_external_db(
        ...
        try:
            ...
            self.logger.info(f"Consultados {len(df)} produtos do banco {config.database}")
    ...
    async def _process_and_insert_products(self, df: pd.DataFrame, empresa_id: int) -> int:
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                existing = self.local_session.query(ProdutoEmpresa).filter(
    ...
    async def get_import_statistics(self, empresa_id: int) -> Dict[str, Any]:
        ...
        try:
            total_products = self.local_session.query(ProdutoEmpresa).filter(
                ProdutoEmpresa.empresa_id == empresa_id
            ).count()
            
            pending_classification = self.local_session.query(ProdutoEmpresa).filter(
                ProdutoEmpresa.empresa_id == empresa_id,
                ProdutoEmpresa.status_classificacao == 'PENDENTE'
            ).count()
            
            classified = self.local_session.query(ProdutoEmpresa).filter(
                ProdutoEmpresa.empresa_id == empresa_id,
                ProdutoEmpresa.status_classificacao.in_(['CONFIRMADO', 'DETERMINADO'])
            ).count()
            
            # Agregações
            unique_descriptions = self.local_session.query(ProdutoEmpresa.descricao_original).filter(
                ProdutoEmpresa.empresa_id == empresa_id
            ).distinct().count()
            ...
            return {
                ...
                "aggregation_ratio": round(unique_descriptions / total_products * 100, 2) if total_products > 0 else 0
            }
    ...
    async def cleanup_and_reimport(self, empresa_id: int) -> bool:
        ...
        try:
            deleted_count = self.local_session.query(ProdutoEmpresa).filter(
                ProdutoEmpresa.empresa_id == empresa_id
            ).delete()
            
            self.local_session.commit()
            self.logger.info(f"Removidos {deleted_count} produtos da empresa {empresa_id}")
...
async def get_external_config_by_company(empresa_id: int) -> Optional[ExternalDatabaseConfig]:
