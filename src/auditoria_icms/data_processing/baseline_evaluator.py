...
class BaselineEvaluator:
        ...
        evaluation_data = [
            {
                ...
                "expected_answer": (
                    "O NCM para medicamentos genéricos é 30049069 - Outros "
                    "medicamentos constituídos por produtos misturados ou não "
                    "misturados"
                ),
                "expected_contexts": [
            ...
            },
            {
                ...
                "expected_answer": (
                    "Telefones celulares (NCM 85171211 e 85171212) sujeitos à "
                    "substituição tributária (CEST 21.001.00)"
                ),
                "expected_contexts": [
            ...
            },
            {
                ...
                "expected_answer": (
                    "Regras NESH definem critérios para classificação de produtos "
                    "importados: origem, composição e finalidade"
                ),
                "expected_contexts": [
            ...
            },
            {
                ...
                "expected_answer": (
                    "Produto requer CEST específica se listado na tabela de produtos "
                    "sujeitos à substituição ou antecipação tributária"
                ),
                "expected_contexts": [
    ...
    def simulate_rag_generation(self, question: str, contexts: List[str]) -> str:
        ...
        if "ncm" in question_lower and "medicamentos" in question_lower:
            return (
                "O NCM para medicamentos genéricos é 30049069 - Outros medicamentos "
                "de produtos misturados ou não misturados"
            )

        elif "telefones" in question_lower or "celulares" in question_lower:
            return (
                "Telefones celulares NCM 85171211/85171212 com substituição tributária "
                "(CEST 21.001.00)"
            )
            ...
            return (
                "Regras NESH: critérios para produtos importados (origem, "
                "composição, finalidade)"
            )

        elif "cest" in question_lower:
            return (
                "Produto requer CEST se listado na tabela de produtos com substituição "
                "ou antecipação tributária"
            )

        else:
            return (
                "Para classificação fiscal, consulte tabelas NCM/CEST atualizadas e "
                "características específicas do produto."
            )

    def calculate_context_precision(
        self, retrieved_contexts: List[str], expected_contexts: List[str]
    ) -> float:
    ...
    def calculate_faithfulness(
        self, generated_answer: str, contexts: List[str]
    ) -> float:
    ...
    def calculate_answer_relevancy(self, generated_answer: str, question: str) -> float:
        ...
        question_words = {
            word for word in question_words if word not in stop_words and len(word) > 2
        }
        answer_words = {
            word for word in answer_words if word not in stop_words and len(word) > 2
        }
    ...
    def calculate_semantic_similarity(
        self, generated_answer: str, expected_answer: str
    ) -> float:
    ...
    def run_baseline_evaluation(self) -> Dict:
        ...
        for i, example in enumerate(self.evaluation_dataset):
            logger.info(
                f"Avaliando exemplo {i+1}/{len(self.evaluation_dataset)}: {example['category']}"
            )
        ...
        for result in results:
            ...
            if category not in category_metrics:
                category_metrics[category] = {
                    metric: [] for metric in self.metrics.keys()
                }
        ...
        evaluation_report = {
            ...
            "summary": {
                "average_context_precision": aggregated_metrics["context_precision"][
                    "mean"
                ],
                "average_context_recall": aggregated_metrics["context_recall"]["mean"],
                "average_faithfulness": aggregated_metrics["faithfulness"]["mean"],
                "average_answer_relevancy": aggregated_metrics["answer_relevancy"][
                    "mean"
                ],
                "average_semantic_similarity": aggregated_metrics[
                    "semantic_similarity"
                ]["mean"],
...
def main():
    ...
    logger.info(
        f"Avaliação concluída com score: {report['summary']['overall_score']:.3f}"
    )
