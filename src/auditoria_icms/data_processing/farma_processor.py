...
class FarmaProcessor:
    ...
    def load_tabela_abc_farma(self) -> pd.DataFrame:
        ...
        try:
            ...
            if not farma_file.exists():
                logger.warning(f"Arquivo {farma_file} não encontrado. Criando dados de exemplo...")
    ...
    def _analyze_description_patterns(self) -> Dict:
        ...
        formas = ['COMPRIMIDO', 'CÁPSULA', 'SOLUÇÃO', 'SUSPENSÃO', 'XAROPE', 'GOTAS', 'POMADA', 'CREME']
        for forma in formas:
            count = self.farma_data['descricao'].str.contains(forma, case=False, na=False).sum()
            if count > 0:
        ...
        for desc in self.farma_data['descricao']:
            matches = re.findall(r'\d+(?:\.\d+)?(?:MG|G|ML|%)', str(desc), re.IGNORECASE)
    ...
    def search_medicamento_by_description(self, descricao: str, similarity_threshold: float = 0.8) -> List[Dict]:
        """
