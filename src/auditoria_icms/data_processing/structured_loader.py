...
try:
    ...
    try:
        ...
        logging.warning("Processadores farmacêutico e NESH não disponíveis")
...
class StructuredDataLoader:
    ...
    def get_ncm_hierarchy(code: str) -> Dict[str, str]:
        ...
        return {"capitulo": code[:2], "posicao": code[:4], "subposicao": code[:6]}
    ...
    def process_ncm_data(self) -> pd.DataFrame:
        ...
        if ncm_df is None:
            logger.warning(
                "Arquivo de tabela NCM não encontrado, criando DataFrame vazio"
            )
            return pd.DataFrame(
                columns=["codigo", "descricao", "capitulo", "posicao", "subposicao"]
            )
        ...
        if "codigo" in ncm_df.columns:
            ...
            ncm_final = ncm_final[
                ["codigo_limpo", "descricao", "capitulo", "posicao", "subposicao"]
            ]
        ...
        if self._farma_data is None:
            ...
            if self.farmaceutico_processor.carregar_dados():
                self._farma_data = {
                    ...
                    "ncm_farmaceutico": list(
                        self.farmaceutico_processor.ncm_farmaceutico
                    ),
                    "cest_farmaceutico": list(
                        self.farmaceutico_processor.cest_farmaceutico
                    ),
                    "estatisticas": self.farmaceutico_processor.get_estatisticas(),
                }
                logger.info(
                    f"Carregados {len(self._farma_data['medicamentos'])} medicamentos"
                )
            else:
                self._farma_data = {}
    ...
    def load_nesh_rules(self) -> Dict:
        ...
        if self._nesh_rules is None:
            ...
            if nesh_data:
                ...
                logger.info(
                    f"Carregadas {len(nesh_data.get('regras_gerais', {}))} regras gerais"
                )
            else:
                self._nesh_rules = {}
    ...
    def buscar_medicamento_por_codigo_barras(
    ...
    def buscar_medicamentos_similares(
        self, descricao: str, limite: int = 5
    ) -> List[Dict]:
        """
    ...
    def run_simple_test(self):
        ...
        if self.processadores_disponíveis:
            ...
            if farma_data:
                logger.info(
                    f"Teste ABC Farma: {len(farma_data.get('medicamentos', {}))} medicamentos"
                )
            ...
            if nesh_data:
                logger.info(
                    f"Teste NESH: {len(nesh_data.get('regras_gerais', {}))} regras gerais"
                )
