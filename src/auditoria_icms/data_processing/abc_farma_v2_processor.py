...
class ABCFarmaV2Processor:
    ...
    def _processar_dados(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Processamento concluído: {self.stats['produtos_unicos']} produtos únicos"
            )
            return True
    ...
    def _limpar_dados(self):
        ...
        # Renomear colunas para padrão (placeholder para futuras transformações)
        # column_mapping removido (era placeholder não utilizado)
        ...
        if "codigo_barras" not in existing_cols:
            ...
            barcode_cols = [
                ...
                or "barras" in col.lower()
                or "gtin" in col.lower()
            ]
            if barcode_cols:
        ...
        self.df_original["descricao"] = (
            self.df_original["descricao"].str.upper().str.strip()
        )

        # Normalizar códigos de barras
        if "codigo_barras" in self.df_original.columns:
            self.df_original["codigo_barras"] = (
                self.df_original["codigo_barras"].astype(str).str.strip()
            )

        # Normalizar NCM
        if "ncm" in self.df_original.columns:
            self.df_original["ncm"] = (
                self.df_original["ncm"].astype(str).str.replace(".", "").str.strip()
            )

        # Normalizar CEST
        if "cest" in self.df_original.columns:
            self.df_original["cest"] = (
                self.df_original["cest"].astype(str).str.replace(".", "").str.strip()
            )
    ...
    def _agregar_produtos(self):
        ...
        self.df_original["id_agregados"] = (
            self.df_original["descricao"].rank(method="dense").astype(int)
        )

        # Contar produtos por descrição
        desc_counts = self.df_original["descricao"].value_counts()
        self.df_original["qtd_mesma_desc"] = self.df_original["descricao"].map(
            desc_counts
        )
    ...
    def agregar_produtos_iguais(self, threshold: float = 0.85) -> Dict:
        ...
        for id1, produto1 in self.medicamentos.items():
            ...
            for id2, produto2 in self.medicamentos.items():
                ...
                if "codigo_produto" in produto1 and "codigo_produto" in produto2:
                    codigo_similar = (
                        produto1["codigo_produto"] == produto2["codigo_produto"]
                    )
        ...
        for i, grupo in enumerate(produtos_para_agregar):
            detalhes_grupo = {
                ...
                "codigos_barras": [
                    p["codigo_barras"] for p in grupo if p["codigo_barras"]
                ],
            }
            relatorio["detalhes_grupos"].append(detalhes_grupo)

        logger.info(
            f"Encontrados {relatorio['grupos_encontrados']} grupos para agregação"
        )
        return relatorio
    ...
    def validar_ncm_cest_consistency(self) -> Dict:
        ...
        ncm_multiplos_cest = {
            ncm: cests for ncm, cests in ncm_cest_mapping.items() if len(cests) > 1
        }
        ...
        relatorio = {
            ...
            "resumo_ncm_cest": {
                ncm: list(cests) for ncm, cests in ncm_cest_mapping.items()
            },
        }

        logger.info(
            f"Validação concluída: {len(inconsistencias)} inconsistências encontradas"
        )
        return relatorio
    ...
    def get_estatisticas_detalhadas(self) -> Dict:
        ...
        return {
            ...
            "distribuicao_agregados": {
                "grupos_1_produto": sum(
                    1 for grupo in self.agregados.values() if len(grupo) == 1
                ),
                "grupos_multiplos": sum(
                    1 for grupo in self.agregados.values() if len(grupo) > 1
                ),
                "maior_grupo": (
                    max(len(grupo) for grupo in self.agregados.values())
                    if self.agregados
                    else 0
                ),
            },
    ...
    def exportar_dados(self, output_dir: str = "data/processed") -> Dict[str, str]:
        ...
        try:
            ...
            logger.info(
                f"Dados exportados para {len(arquivos_gerados)} arquivos em {output_dir}"
            )
