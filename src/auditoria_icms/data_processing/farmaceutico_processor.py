...
class FarmaceuticoProcessor:
    ...
    def _processar_medicamentos(self, df: pd.DataFrame):
        ...
        for _, row in df.iterrows():
            ...
            medicamento["descricao_enriquecida"] = self._criar_descricao_enriquecida(
                medicamento
            )
    ...
    def _criar_descricao_enriquecida(self, medicamento: Dict) -> str:
        ...
        descricao_enriquecida += "Medicamento farmacêutico do capítulo 30 NCM. "
        ...
        descricao_enriquecida += (
            "Produto sujeito à substituição tributária (CEST segmento 13)."
        )
    ...
    def get_estatisticas(self) -> Dict:
        ...
        return {
            ...
            "laboratorios": list(
                set(med["laboratorio"] for med in self.medicamentos.values())
            ),
            "formas_farmaceuticas": list(
                set(med["forma_farmaceutica"] for med in self.medicamentos.values())
            ),
            "principios_ativos": list(
                set(med["principio_ativo"] for med in self.medicamentos.values())
            ),
        }

    def exportar_para_json(
        self, output_path: str = "data/processed/medicamentos_abc_farma.json"
    ):
        """
