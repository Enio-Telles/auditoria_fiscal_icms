...
from ..database.models import ProdutoEmpresa, Empresa
...
class DatabaseConnector:
    ...
    def _build_connection_string(self) -> str:
        ...
        if self.config.additional_params:
            params = "&".join([f"{k}={v}" for k, v in self.config.additional_params.items()])
    ...
    def connect(self) -> Engine:
        ...
        try:
            ...
            logger.info(f"Conexão estabelecida com {self.config.db_type} em {self.config.host}")
            return self.engine
...
class EmpresaDataIngestion:
    ...
    def extract_produtos(self, 
                        limit: Optional[int] = None,
                        offset: Optional[int] = 0,
                        where_clause: Optional[str] = None) -> List[ProdutoEmpresa]:
        """
        ...
        try:
            ...
            logger.info(f"Extraídos {len(produtos)} produtos da empresa {self.empresa_id}")
            return produtos
            
        except Exception as e:
            logger.error(f"Erro ao extrair produtos da empresa {self.empresa_id}: {str(e)}")
            raise
        finally:
    ...
    def update_produtos_processados(self, produtos_atualizados: List[Dict[str, Any]]) -> bool:
        """
        ...
        try:
            ...
            with engine.begin() as conn:
                for produto in produtos_atualizados:
                    ...
                    conn.execute(text(update_query), {
                        ...
                        'descricao_enriquecida': produto.get('descricao_enriquecida'),
                        'ncm_sugerido': produto.get('ncm_sugerido'),
                        'cest_sugerido': produto.get('cest_sugerido'),
                        'status_processamento': produto.get('status_processamento', 'PROCESSADO'),
            ...
            logger.info(f"Atualizados {len(produtos_atualizados)} produtos no banco da empresa {self.empresa_id}")
            return True
            
        except Exception as e:
            logger.error(f"Erro ao atualizar produtos da empresa {self.empresa_id}: {str(e)}")
            return False
        finally:
            self.connector.disconnect()
...
class DataIngestionManager:
    ...
    def get_empresa_ingestion(self, empresa_id: int, db_config: Dict[str, Any]) -> EmpresaDataIngestion:
        """Obtém ou cria uma instância de ingestão para uma empresa"""
        
        if empresa_id not in self.active_connections:
            self.active_connections[empresa_id] = EmpresaDataIngestion(empresa_id, db_config)
    ...
    def extract_produtos_bulk(self, 
                             empresa_configs: List[Dict[str, Any]], 
                             batch_size: int = 1000) -> Dict[int, List[ProdutoEmpresa]]:
        ...
        for config in empresa_configs:
            ...
            try:
                ...
                logger.error(f"Erro ao extrair produtos da empresa {empresa_id}: {str(e)}")
                results[empresa_id] = []
