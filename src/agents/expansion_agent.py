...
class ExpansionAgent(BaseAgent):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.logger.info(
            "ExpansionAgent inicializado com capacidades de enriquecimento"
        )
    ...
    async def _expand_description(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # product_code removido (não utilizado)
        ...
        result = {
            ...
            "expansion_ratio": len(expanded_description)
            / max(1, len(original_description)),
            "processing_metadata": {
                "agent": self.name,
    ...
    def _extract_technical_features(self, text: str) -> List[str]:
        ...
        for pattern_name, pattern_config in self.technical_patterns.items():
            ...
            if matches:
                features.extend(
                    [f"{pattern_name}: {match}" for match in matches[:2]]
                )  # Limite 2 por tipo
    ...
    def _find_relevant_synonyms(self, text: str) -> List[str]:
        ...
        for word in words:
            if word in self.synonyms_db:
                synonyms.extend(
                    self.synonyms_db[word][:2]
                )  # Máximo 2 sinônimos por palavra
    ...
    def _calculate_expansion_confidence(
        self, original: str, expanded: str, features: List[str]
    ) -> float:
    ...
    def _find_pattern_matches(
        self, text: str, pattern_config: Dict[str, Any]
    ) -> List[str]:
    ...
    def _extract_brand_indicators(self, text: str) -> List[str]:
        ...
        brand_patterns = [r"marca\s+(\w+)", r"fabricante\s+(\w+)", r"®", r"™"]
    ...
    def _generate_classification_hints(self, data: Dict[str, Any]) -> List[str]:
        ...
        if any(
            word in description for word in ["medicamento", "remédio", "farmacêutico"]
        ):
    ...
    def _suggest_data_improvements(self, data: Dict[str, Any]) -> List[str]:
        ...
        if len(description) < self.config["min_description_length"]:
            suggestions.append(
                "Descrição muito curta - considere adicionar mais detalhes"
            )

        if not any(char.isdigit() for char in description):
            suggestions.append(
                "Considere adicionar especificações numéricas (dimensões, peso, etc.)"
            )

        if not data.get("category"):
            suggestions.append(
                "Categoria não informada - importante para classificação precisa"
            )
