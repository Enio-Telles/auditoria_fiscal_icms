...
class CESTAgent(BaseAgent):
    ...
    async def _classify_cest(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 6. Selecionar melhor candidato
        best_classification = self._select_best_cest(all_candidates, full_text, ncm_code)
        
        # 7. Verificar exigência de ST
        st_analysis = self._analyze_st_requirement_for_cest(best_classification["code"], state)
        
        # 8. Gerar explicação
        explanation = self._generate_cest_explanation(
            best_classification, ncm_code, st_analysis
        )
        
        # 9. Sugerir alternativas
        alternatives = self._generate_cest_alternatives(all_candidates, best_classification)
        
        result = {
            "cest_code": best_classification["code"],
            "cest_description": best_classification["description"],
            ...
            "ncm_cest_compatibility": self._check_ncm_cest_compatibility(ncm_code, best_classification["code"]),
            "processing_metadata": {
                "agent": self.name,
        ...
        self.logger.info(f"CEST classificado: {best_classification['code']} (confiança: {best_classification['confidence']:.2f})")
        return result
    ...
    async def _validate_cest(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        description_compatibility = self._validate_cest_description_compatibility(cest_code, description)
        ...
        validation_score = self._calculate_cest_validation_score(
            format_validation, existence_validation, ncm_compatibility, 
            description_compatibility, state_applicability
        )
    ...
    async def _map_ncm_to_cest(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for cest_code in related_cests:
            ...
            enriched_cests.append({
                ...
                "confidence": self._calculate_ncm_cest_confidence(ncm_code, cest_code)
            })
        
        # Ordenar por confiança
        enriched_cests = sorted(enriched_cests, key=lambda x: x["confidence"], reverse=True)
        ...
        result = {
            ...
            "mapping_confidence": self._calculate_mapping_confidence(ncm_code, enriched_cests),
    ...
    async def _analyze_st_requirement(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Analisa exigência de substituição tributária.
        
        Args:
            data: Dados do produto e localização
        ...
        # Obter informações do CEST
        cest_info = self.cest_database.get(cest_code, {})
        
        # Verificar se ST é aplicável
        st_analysis = {
            "cest_code": cest_code,
            "state": state,
    ...
    async def _suggest_alternatives(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            data: Dados do produto ou CEST atual
            
        Returns:
            Lista de CESTs alternativos
        """
        ...
        # 1. Alternativas baseadas no NCM
        if ncm_code:
            ncm_alternatives = await self._map_ncm_to_cest({"ncm_code": ncm_code})
            if ncm_alternatives.get("related_cests"):
                alternatives.extend(ncm_alternatives["related_cests"])
        
        # 2. Alternativas baseadas na descrição
        if description:
            description_alternatives = await self._classify_cest({"description": description})
    ...
    async def _detect_inconsistencies(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Detecta inconsistências em classificações CEST.
        
        Args:
            data: Dados de produtos com CESTs para análise
        ...
        self.logger.info(f"Detectando inconsistências CEST em {len(products)} produtos")
        
        inconsistencies = []
        
        # 1. Inconsistências NCM-CEST
        ncm_cest_inconsistencies = self._detect_ncm_cest_inconsistencies(products)
        inconsistencies.extend(ncm_cest_inconsistencies)
        
        # 2. Inconsistências de ST
        st_inconsistencies = self._detect_st_inconsistencies(products)
        inconsistencies.extend(st_inconsistencies)
        
        # 3. Inconsistências por similaridade de descrição
        description_inconsistencies = self._detect_cest_description_inconsistencies(products)
        inconsistencies.extend(description_inconsistencies)
        
        # 4. Classificar por severidade
        classified_inconsistencies = self._classify_cest_inconsistencies(inconsistencies)
        ...
        result = {
            ...
            "summary": {
                "by_type": self._count_inconsistencies_by_type(inconsistencies),
                "by_severity": self._count_inconsistencies_by_severity(classified_inconsistencies)
            },
            "recommendations": self._generate_cest_inconsistency_recommendations(classified_inconsistencies)
        }
    ...
    def _combine_cest_results(self, ncm_matches: List, keyword_matches: List, semantic_matches: List, state_matches: List) -> List[Dict[str, Any]]:
    ...
    def _select_best_cest(self, candidates: List[Dict[str, Any]], text: str, ncm_code: str) -> Dict[str, Any]:
        ...
        sorted_candidates = sorted(
            candidates, 
            key=lambda x: (
                1 if "ncm_mapping" in x["methods"] else 0,
                x["confidence"]
            ), 
            reverse=True
        )
    ...
    def _analyze_st_requirement_for_cest(self, cest_code: str, state: str) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "applicable_in_state": not state or state in applicable_states or not applicable_states,
    ...
    def _generate_cest_explanation(self, classification: Dict[str, Any], ncm_code: str, st_analysis: Dict[str, Any]) -> str:
    ...
    def _generate_cest_alternatives(self, all_candidates: List[Dict[str, Any]], best_classification: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _check_ncm_cest_compatibility(self, ncm_code: str, cest_code: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "reason": "NCM compatível com CEST" if is_compatible else "NCM não relacionado ao CEST",
    ...
    def _validate_cest_ncm_compatibility(self, cest_code: str, ncm_code: str) -> Dict[str, Any]:
    ...
    def _validate_cest_description_compatibility(self, cest_code: str, description: str) -> Dict[str, Any]:
        """Valida compatibilidade CEST com descrição do produto."""
        if not description:
            return {"compatible": True, "score": 0.5, "reason": "Descrição não fornecida"}
        ...
        # Calcular similaridade
        description_similarity = SequenceMatcher(None, description.lower(), cest_description.lower()).ratio()
    ...
    def _validate_cest_state_applicability(self, cest_code: str, state: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "reason": f"Aplicável no estado {state}" if is_applicable else f"Não aplicável no estado {state}",
            "applicable_states": applicable_states
        }
    
    def _calculate_cest_validation_score(self, format_val: Dict, existence_val: Dict, 
                                       ncm_comp: Dict, desc_comp: Dict, state_app: Dict) -> float:
    ...
    def _generate_cest_validation_recommendations(self, cest_code: str, validation_score: float,
                                                format_val: Dict, existence_val: Dict) -> List[str]:
        """Gera recomendações de validação CEST."""
        recommendations = []
        
        if validation_score < 0.7:
            if not format_val["is_valid"]:
                recommendations.append("Corrigir formato do código CEST (deve ter 7 dígitos)")
            
            if not existence_val["exists"]:
                recommendations.append("Verificar se o código CEST está correto - não encontrado na base")
            
            if validation_score < 0.5:
                recommendations.append("Revisar completamente a classificação CEST")
            elif validation_score < 0.7:
                recommendations.append("Validar novamente alguns aspectos da classificação")
        else:
    ...
    def _analyze_st_implications(self, cests: List[Dict], state: str) -> Dict[str, Any]:
        ...
        if state:
            state_applicable = sum(1 for cest in cests 
                                 if not cest.get("applicable_states") or state in cest.get("applicable_states", []))
            implications["state_applicable_count"] = state_applicable
            implications["state_applicability_percentage"] = (state_applicable / len(cests)) * 100 if cests else 0
    ...
    def _calculate_st_tax_impact(self, cest_info: Dict, operation_type: str) -> Dict[str, Any]:
        ...
        if st_analysis.get("st_required"):
            if st_analysis.get("applicable_in_state"):
                ...
                if st_analysis.get("is_exempt"):
                    recommendations.append("Operação pode estar isenta - verificar condições")
            else:
            ...
            if similarity_score > 0.3:
                similar.append({
                    ...
                    "similarity_factors": {
                        ...
                        "keyword_similarity": keyword_similarity if base_keywords else 0
                    }
                })
        
        return sorted(similar, key=lambda x: x["confidence"], reverse=True)
    
    def _deduplicate_and_rank_cest_alternatives(self, alternatives: List[Dict]) -> List[Dict]:
        ...
        for alt in alternatives:
            ...
            if code not in unique_alternatives:
                ...
                if alt.get("confidence", 0) > unique_alternatives[code].get("confidence", 0):
                    unique_alternatives[code] = alt
    ...
    def _detect_st_inconsistencies(self, products: List[Dict]) -> List[Dict]:
        ...
        for product in products:
            ...
            if cest_code:
                ...
                should_have_st = st_analysis.get("st_required", False) and st_analysis.get("applicable_in_state", True)
    ...
    def _detect_cest_description_inconsistencies(self, products: List[Dict]) -> List[Dict]:
                    ...
                    for j, desc2 in enumerate(descriptions[i+1:], i+1):
                        ...
                        if similarity < 0.3:  # Baixa similaridade
                            inconsistencies.append({
                                ...
                                "cest_code": cest_code,
                                "product_ids": [group_products[i].get("id"), group_products[j].get("id")],
                                "descriptions": [desc1, desc2],
                                "similarity": similarity,
                                "description": f"Produtos com mesmo CEST têm descrições muito diferentes",
                                "severity": "low"
                            })
    ...
    def _classify_cest_inconsistencies(self, inconsistencies: List[Dict]) -> List[Dict]:
        ...
        return sorted(inconsistencies, key=lambda x: severity_order.get(x.get("severity", "low"), 1), reverse=True)
    
    def _count_inconsistencies_by_type(self, inconsistencies: List[Dict]) -> Dict[str, int]:
    ...
    def _count_inconsistencies_by_severity(self, inconsistencies: List[Dict]) -> Dict[str, int]:
    ...
    def _generate_cest_inconsistency_recommendations(self, inconsistencies: List[Dict]) -> List[str]:
        ...
        if high_severity:
            recommendations.append(f"Corrigir urgentemente {len(high_severity)} inconsistências de alta severidade")
        
        if medium_severity:
            recommendations.append(f"Revisar {len(medium_severity)} inconsistências de média severidade")
        ...
        if type_counts.get("ncm_cest_incompatibility", 0) > 0:
            recommendations.append("Verificar compatibilidade NCM-CEST nos produtos identificados")
    ...
    async def _compare_cest_codes(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(cest_codes) < 2:
            return {"error": "Pelo menos dois códigos CEST são necessários para comparação"}
        ...
        comparison["similarities"], comparison["differences"] = self._analyze_cest_similarities_differences(
    ...
    async def _extract_cest_keywords(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        product_patterns = {
            ...
            "medicamentos": ["medicamento", "remédio", "comprimido", "cápsula", "xarope"],
        ...
        # Indicadores de ST
        st_indicators = ["substituição", "tributária", "st", "margem", "valor", "agregado", "mva"]
        cest_keywords["st_indicators"] = [kw for kw in st_indicators if kw in text_lower]
        ...
        return {
            ...
            "extraction_confidence": self._calculate_keyword_extraction_confidence(cest_keywords),
    ...
    def _get_cest_common_applications(self, cest_code: str) -> List[str]:
        ...
        common_apps = {
            "0100100": ["Pneus para veículos", "Pneus para motocicletas", "Pneus para bicicletas"],
            "1700100": ["Medicamentos genéricos", "Medicamentos de marca", "Antibióticos"]
        }
    ...
    def _analyze_cest_similarities_differences(self, detailed_comparison: List[Dict]) -> Tuple[List[str], List[str]]:
        ...
        first_cest = detailed_comparison[0]
        
        # ST requirement
        st_requirements = [cest.get("st_required", False) for cest in detailed_comparison]
        if all(st_requirements) or not any(st_requirements):
            similarities.append(f"Todos {'requerem' if st_requirements[0] else 'não requerem'} substituição tributária")
        else:
    ...
    def _generate_cest_comparison_recommendations(self, similarities: List[str], differences: List[str]) -> List[str]:
        """Gera recomendações baseadas na comparação."""
        recommendations = []
        
        if similarities:
            recommendations.append("CESTs têm características similares - verificar se produtos são realmente diferentes")
        
        if differences:
            recommendations.append("CESTs têm diferenças significativas - confirmar classificação adequada")
        
        recommendations.append("Verificar documentação específica de cada CEST")
        recommendations.append("Consultar legislação estadual para confirmar aplicabilidade")
    ...
    def _calculate_keyword_extraction_confidence(self, keywords: Dict) -> float:
        ...
        total_keywords = sum(len(keywords[key]) for key in keywords if isinstance(keywords[key], list))
        ...
        for key, weight in weights.items():
            if key in keywords and isinstance(keywords[key], list):
                ...
                weighted_score += min(keyword_count / 5, 1.0) * weight  # Máximo 5 keywords por categoria
    ...
    def _summarize_keyword_analysis(self, keywords: Dict) -> str:
        ...
        if st_indicators:
            summaries.append(f"Encontrados {len(st_indicators)} termos relacionados à ST")
