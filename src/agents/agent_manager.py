...
class AgentManager:
    ...
    def register_agent_type(
        self, agent_type: Type[BaseAgent], name: Optional[str] = None
    ) -> None:
        """
    ...
    async def create_agent(
        self, agent_type: str, agent_name: str, config: Optional[Dict[str, Any]] = None
    ) -> str:
        """
    ...
    async def execute_task(self, agent_name: str, task: AgentTask) -> Dict[str, Any]:
        ...
        if agent.status != AgentStatus.RUNNING:
            raise RuntimeError(
                f"Agente {agent_name} não está disponível (status: {agent.status})"
            )
        ...
        try:
            ...
            result = await asyncio.wait_for(
                agent.process_task(task), timeout=self.task_timeout
            )
            ...
            raise RuntimeError(
                f"Timeout na execução da tarefa {task.type} no agente {agent_name}"
            )

        except Exception as e:
            execution_time = (datetime.now() - start_time).total_seconds()
            await self._record_task_failure(
                agent_name, task.type, execution_time, str(e)
            )
            raise

    async def execute_task_with_retry(
        self, agent_name: str, task: AgentTask, max_retries: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt > 0:
                    self.logger.info(
                        f"Tarefa {task.type} executada com sucesso após {attempt} tentativas"
                    )
                ...
                if attempt < max_retries:
                    ...
                    self.logger.warning(
                        f"Tentativa {attempt + 1} falhou para {task.type}, tentando novamente em {wait_time}s: {e}"
                    )
                    await asyncio.sleep(wait_time)
                else:
                    self.logger.error(
                        f"Todas as {max_retries + 1} tentativas falharam para {task.type}"
                    )
    ...
    def get_agent_info(self, agent_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "config": agent.config,
            "created_at": (
                agent.created_at.isoformat() if hasattr(agent, "created_at") else None
            ),
        }
    ...
    def get_system_metrics(self) -> Dict[str, Any]:
        ...
        total_tasks = sum(
            metrics["tasks_completed"] for metrics in self.metrics.values()
        )
        total_failures = sum(
            metrics["tasks_failed"] for metrics in self.metrics.values()
        )

        success_rate = (
            (total_tasks / (total_tasks + total_failures)) * 100
        ...
        for agent_name in self.performance_history:
            all_response_times.extend(
                [p["execution_time"] for p in self.performance_history[agent_name]]
            )
        ...
        return {
            "system_overview": {
                ...
                "uptime_percentage": (
                    (running_agents / total_agents) * 100 if total_agents > 0 else 0
                ),
            },
            "task_metrics": {
    ...
    async def _check_all_agents_health(self) -> None:
        ...
        for agent_name, agent in self.agents.items():
            try:
                ...
                self.logger.error(
                    f"Erro ao verificar saúde do agente {agent_name}: {e}"
                )
    ...
    async def _handle_unhealthy_agent(self, agent_name: str, agent: BaseAgent) -> None:
        ...
        self.logger.warning(
            f"Agente {agent_name} está não saudável, tentando recuperar"
        )
    ...
    async def _collect_agent_metrics(self) -> None:
        ...
        for agent_name, agent in self.agents.items():
            try:
                ...
                self.logger.error(
                    f"Erro ao coletar métricas do agente {agent_name}: {e}"
                )

    def _update_agent_metrics(
        self, agent_name: str, new_metrics: Dict[str, Any]
    ) -> None:
    ...
    async def _record_task_success(
        self, agent_name: str, task_type: str, execution_time: float
    ) -> None:
    ...
    async def _record_task_failure(
        self, agent_name: str, task_type: str, execution_time: float, error: str
    ) -> None:
    ...
    def _get_performance_summary(self, agent_name: str) -> Dict[str, Any]:
        ...
        success_count = len(
            [entry for entry in recent_history if entry["status"] == "success"]
        )
    ...
    async def _validate_agent_config(
        self, agent: BaseAgent, config: Dict[str, Any]
    ) -> None:
