...
class BaseAgent(ABC):
    ...
    async def add_task(self, task: AgentTask) -> str:
        ...
        self.logger.info(
            f"Tarefa {task.id} adicionada à fila (prioridade: {task.priority.name})"
        )
        return task.id
    ...
    async def process_next_task(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if task.retries < task.max_retries:
                self.logger.info(
                    f"Recolocando tarefa {task.id} na fila (tentativa {task.retries + 1})"
                )
                self.tasks_queue.insert(0, task)  # Prioridade alta para retry
            else:
                self.logger.error(
                    f"Tarefa {task.id} falhou após {task.max_retries} tentativas"
                )
    ...
    async def send_message(
        self, receiver: str, message_type: str, payload: Dict[str, Any]
    ) -> str:
        """
        ...
        message = AgentMessage(
            sender=self.name, receiver=receiver, type=message_type, payload=payload
        )
    ...
    def get_task_history(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        """
        recent_tasks = (
            self.completed_tasks[-limit:] if limit > 0 else self.completed_tasks
        )
