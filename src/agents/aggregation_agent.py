...
class AggregationAgent(BaseAgent):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.logger.info(
            "AggregationAgent inicializado com capacidades de consolidação"
        )
    ...
    async def _consolidate_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Consolidando {len(sources)} fontes com estratégia '{consolidation_strategy}'"
        )
        ...
        if consolidation_strategy == "merge":
            ...
            raise ValueError(
                f"Estratégia de consolidação não suportada: {consolidation_strategy}"
            )
        ...
        result = {
            ...
            "processing_metadata": {
                ...
                "total_records": (
                    len(consolidated) if isinstance(consolidated, list) else 1
                ),
            },
        }

        self.logger.info(
            f"Consolidação concluída: {len(consolidated) if isinstance(consolidated, list) else 1} registros"
        )
        return result
    ...
    async def _detect_patterns(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        pattern_types = data.get(
            "pattern_types", ["frequency", "sequence", "correlation"]
        )
    ...
    async def _generate_report(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        dataset = data.get("dataset", [])
        # include_charts removido (não utilizado)
        ...
        report_sections["patterns"] = await self._detect_patterns(
            {"dataset": dataset, "pattern_types": ["frequency", "correlation"]}
        )
        ...
        if self.config["enable_anomaly_detection"]:
            report_sections["anomalies"] = await self._detect_anomalies(
                {"dataset": dataset}
            )
    ...
    async def _analyze_trends(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Analisando tendências em {len(value_fields) or 'todos os campos'} campos"
        )
        ...
        result = {
            ...
            "trend_summary": {
                "trending_up": [
                    f for f, t in trends.items() if t.get("direction") == "up"
                ],
                "trending_down": [
                    f for f, t in trends.items() if t.get("direction") == "down"
                ],
                "stable": [
                    f for f, t in trends.items() if t.get("direction") == "stable"
                ],
            },
        }
    ...
    async def _calculate_metrics(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for metric_type in metric_types:
            if metric_type == "accuracy":
                calculated_metrics[metric_type] = self._calculate_accuracy_metrics(
                    dataset
                )
                ...
                calculated_metrics[metric_type] = self._calculate_consistency_metrics(
                    dataset
                )
            elif metric_type == "diversity":
                calculated_metrics[metric_type] = self._calculate_diversity_metrics(
                    dataset
                )
            elif metric_type == "distribution":
                calculated_metrics[metric_type] = self._calculate_distribution_metrics(
                    dataset
                )
    ...
    async def _intersection_sources(
        self, sources: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    def _calculate_consolidation_stats(
        self, sources: List[Dict[str, Any]], consolidated: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    async def _grouped_statistics(
        self, dataset: List[Dict[str, Any]], metrics: List[str], group_by: str
    ) -> Dict[str, Any]:
    ...
    async def _overall_statistics(
        self, dataset: List[Dict[str, Any]], metrics: List[str]
    ) -> Dict[str, Any]:
        ...
        for field in numeric_fields:
            values = [
                item.get(field) for item in dataset if item.get(field) is not None
            ]
    ...
    def _calculate_additional_metrics(
        ...
        return {
            ...
            "duplicate_rate": 1
            - (len(set(str(item) for item in dataset)) / max(1, len(dataset))),
    ...
    def _detect_correlation_patterns(
        ...
        for i, field1 in enumerate(numeric_fields):
            for field2 in numeric_fields[i + 1 :]:
                values1 = [
                    item.get(field1) for item in dataset if item.get(field1) is not None
                ]
                values2 = [
                    item.get(field2) for item in dataset if item.get(field2) is not None
                ]
    ...
    def _calculate_pattern_confidence(
        ...
        for pattern_type, pattern_list in patterns.items():
            if pattern_type == "frequency":
                ...
                scores = [
                    abs(p.get("correlation_coefficient", 0)) for p in pattern_list
                ]
            else:
    ...
    def _calculate_consistency_metrics(
        ...
        return {**consistency_scores, "overall": overall}
    ...
    def _determine_data_period(self, dataset: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {"status": "analysis_period", "records_analyzed": len(dataset)}
