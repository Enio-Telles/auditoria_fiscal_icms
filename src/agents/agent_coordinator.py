...
class AgentCoordinator:
    ...
    def __init__(
        self, agent_manager: AgentManager, config: Optional[Dict[str, Any]] = None
    ):
        """
    ...
    def register_workflow_template(
        self, template_name: str, template_definition: Dict[str, Any]
    ) -> None:
        """
    ...
    async def create_workflow_from_template(
        self, template_name: str, workflow_id: str, context: Dict[str, Any]
    ) -> str:
        """
        ...
        self.logger.info(
            f"Workflow criado a partir do template {template_name}: {workflow_id}"
        )
        return workflow_id
    ...
    def get_workflow_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance": {
                ...
                "success_rate": (
                    (status_counts["completed"] / total_workflows) * 100
            ...
            "resource_usage": {
                ...
                "usage_percentage": (running_workflows / self.max_concurrent_workflows)
                * 100,
            },
        }
    ...
    async def _execute_workflow_internal(self, workflow: Workflow) -> None:
        ...
        try:
            ...
            if failed_steps and workflow.failure_strategy == "stop":
                ...
                workflow.error = (
                    f"Workflow falhou devido a {len(failed_steps)} steps com falha"
                )
            else:
            ...
            self.logger.info(
                f"Workflow concluído: {workflow.id} (status: {workflow.status.value})"
            )
        ...
        while True:
            ...
            if not ready_steps:
                ...
                pending_steps = [
                    ...
                    if s.status in [StepStatus.WAITING, StepStatus.RUNNING]
                ]
            ...
            for step in ready_steps:
                task = asyncio.create_task(
                    self._execute_step_with_semaphore(semaphore, workflow, step)
                )
                tasks.append(task)
    ...
    def _find_ready_steps(
        self, workflow: Workflow, completed_steps: set
    ) -> List[WorkflowStep]:
    ...
    async def _execute_step_with_semaphore(
        self, semaphore: asyncio.Semaphore, workflow: Workflow, step: WorkflowStep
    ) -> None:
    ...
    async def _execute_step(self, workflow: Workflow, step: WorkflowStep) -> None:
        ...
        for attempt in range(step.retry_attempts + 1):
            ...
            try:
                ...
                # Criar tarefa
                task = AgentTask(
                    type=step.task_type, data=task_data, priority=TaskPriority.NORMAL
                )
        ...
        step.completed_at = datetime.now()
        self.logger.error(
            f"Step {step.id} falhou após {step.retry_attempts + 1} tentativas"
        )
    ...
    def _update_workflow_context(
        self, workflow: Workflow, step: WorkflowStep, result: Dict[str, Any]
    ) -> None:
    ...
    def _build_workflow_from_template(
        self, template: Dict[str, Any], workflow_id: str, context: Dict[str, Any]
    ) -> Workflow:
            ...
            if workflow.status == WorkflowStatus.RUNNING:
                ...
                if workflow.global_timeout and workflow.started_at:
                    ...
                    if elapsed > workflow.global_timeout:
                        self.logger.warning(
                            f"Workflow {workflow_id} excedeu timeout global"
                        )
