...
class NCMAgent(BaseAgent):
    ...
    async def _classify_ncm(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if product_category:
            hierarchical_matches = self._hierarchical_analysis(
                product_category, full_text
            )
        ...
        alternatives = self._generate_ncm_alternatives(
            all_candidates, best_classification
        )
        ...
        self.logger.info(
            f"NCM classificado: {best_classification['code']} (confiança: {best_classification['confidence']:.2f})"
        )
        return result
    ...
    async def _validate_ncm(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        compatibility_validation = self._validate_ncm_compatibility(
            ncm_code, description
        )
    ...
    async def _suggest_alternatives(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if current_ncm:
            hierarchical_alternatives = self._find_hierarchical_alternatives(
                current_ncm
            )
        ...
        if description:
            description_alternatives = await self._classify_ncm(
        ...
        result = {
            ...
            "confidence_range": {
                "min": (
                    min(alt["confidence"] for alt in limited_alternatives)
                    if limited_alternatives
                    else 0
                ),
                "max": (
                    max(alt["confidence"] for alt in limited_alternatives)
                    if limited_alternatives
                    else 0
                ),
            },
    ...
    async def _detect_inconsistencies(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        hierarchical_inconsistencies = self._detect_hierarchical_inconsistencies(
            products
        )
    ...
    async def _explain_classification(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        compatibility_analysis = self._analyze_classification_compatibility(
            ncm_code, description
        )
    ...
    async def _compare_ncm_codes(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = {
            ...
            "clustering_suggestions": self._suggest_ncm_clustering(
                ncm_codes, similarity_matrix
            ),
        }
    ...
    def _build_ncm_hierarchy(self) -> Dict[str, Dict[str, Any]]:
        ...
        for code, info in self.ncm_database.items():
            ...
            if heading not in hierarchy[chapter]["headings"]:
                hierarchy[chapter]["headings"][heading] = {
                    "codes": [],
                    "description": f"Posição {heading}",
                }
    ...
    def _combine_classification_results(
        self, keyword_matches: List, semantic_matches: List, hierarchical_matches: List
    ) -> List[Dict[str, Any]]:
        ...
        for match in hierarchical_matches:
            ...
            if len(combined[code]["methods"]) > 0:
                combined[code]["confidence"] = min(
                    combined[code]["confidence"] + 0.2, 1.0
                )
            else:
                combined[code]["confidence"] = match["confidence"]
    ...
    def _select_best_ncm(
        self, candidates: List[Dict[str, Any]], text: str
    ) -> Dict[str, Any]:
    ...
    def _generate_ncm_alternatives(
    ...
    def _validate_ncm_compatibility(
        self, ncm_code: str, description: str
    ) -> Dict[str, Any]:
        ...
        similarity = SequenceMatcher(
            None, description.lower(), ncm_description.lower()
        ).ratio()
    ...
    def _calculate_validation_score(
        self, format_val: Dict, existence_val: Dict, compatibility_val: Dict
    ) -> float:
        ...
        return (
            (format_score * 0.2) + (existence_score * 0.3) + (compatibility_score * 0.5)
        )

    def _generate_validation_recommendations(
        self, ncm_code: str, description: str, score: float
    ) -> List[str]:
        ...
        if score < 0.3:
            recommendations.append(
                "Código NCM provavelmente incorreto - revisar classificação"
            )
        elif score < 0.7:
            recommendations.append(
                "Código NCM questionável - verificar compatibilidade"
            )
        else:
        ...
        if ncm_code not in self.ncm_database:
            recommendations.append(
                "Verificar se código NCM existe na nomenclatura oficial"
            )
