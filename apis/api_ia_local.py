...
from auditoria_icms.ai.local_llm_classifier import (
    LocalLLMClassifier,
)
...
async def update_config(
    config: AIConfigUpdate, classifier: LocalLLMClassifier = Depends(get_classifier)
):
...
async def process_batch_background(
    ...
    try:
        ...
        if salvar:
            filename = classifier.export_results(
                results, f"data/processed/batch_{job_id}.json"
            )
...
async def get_analytics():
    ...
    try:
        ...
        tempo_medio = (
            tempo_total / total_classificacoes if total_classificacoes > 0 else 0
        )

        return {
            "total_classificacoes": total_classificacoes,
            "arquivos_processados": len(classification_files) + len(batch_files),
            "tempo_total_processamento": tempo_total,
            "tempo_medio_por_item": tempo_medio,
            "distribuicao_modelos": modelos_usados,
        }

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Erro ao processar estatísticas: {str(e)}"
        )
...
if __name__ == "__main__":
    ...
    print("🚀 Iniciando API de IA Local em modo de desenvolvimento...")
    print("Acesse a documentação em http://127.0.0.1:8001/docs")
