...
def get_db_connection(database_name: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Erro de conexão com banco {database_name}: {str(e)}",
        )
    ...
    try:
        if connection.type == "sqlserver":
            if not PYODBC_AVAILABLE:
                raise HTTPException(
                    status_code=501,
                    detail="SQL Server requer pyodbc. Instale com: pip install pyodbc",
                )

            conn_str = (
                f"DRIVER={{ODBC Driver 17 for SQL Server}};SERVER={connection.host},{connection.port};"
                f"DATABASE={connection.database};UID={connection.user};PWD={connection.password}"
            )
            ...
            if not MYSQL_AVAILABLE:
                raise HTTPException(
                    status_code=501,
                    detail="MySQL requer mysql-connector-python. Instale com: pip install mysql-connector-python",
                )
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Tipo de banco não suportado: {connection.type}",
            )

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Erro ao conectar ao banco externo: {str(e)}"
        )
            ...
            for row in rows:
                try:
                    ...
                    if config.update_existing:
                        # Lógica de update
                        query = (
                            "INSERT INTO produtos (nome, descricao, codigo_produto, codigo_barra, "
                            "ncm_codigo, cest_codigo, criado_em) VALUES ("
                            "%(nome)s, %(descricao)s, %(codigo_produto)s, %(codigo_barra)s, "
                            "%(ncm_codigo)s, %(cest_codigo)s, NOW()) ON CONFLICT (codigo_produto) "
                            "DO UPDATE SET descricao = EXCLUDED.descricao, ncm_codigo = EXCLUDED.ncm_codigo, "
                            "cest_codigo = EXCLUDED.cest_codigo, atualizado_em = NOW()"
                        )
                    else:
                        ...
                        query = (
                            ...
                            "ncm_codigo, cest_codigo, criado_em) VALUES ("
                            "%(nome)s, %(descricao)s, %(codigo_produto)s, %(codigo_barra)s, "
                            "%(ncm_codigo)s, %(cest_codigo)s, NOW()) "
...
def get_empresa_database(empresa_id: int) -> str:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro ao obter banco da empresa: {str(e)}"
        )
...
async def classificar_produto(empresa_id: int, request: ClassificacaoRequest):
    ...
    if (
        "farmac" in produto["nome"].lower()
        or "medicamento" in (produto["descricao"] or "").lower()
    ):
        ncm_sugerido = "30049099"
        cest_sugerido = "2800100"
    elif (
        "smartphone" in produto["nome"].lower() or "celular" in produto["nome"].lower()
    ):
    ...
    classificacao_query = (
        "INSERT INTO classificacoes_ia (produto_id, ncm_sugerido, ncm_confianca, "
        "cest_sugerido, cest_confianca, justificativa, agente_usado, status) "
        "VALUES (%s, %s, %s, %s, %s, %s, %s, %s) RETURNING *"
    )

    classificacao_result = execute_query(
        database_name,
        classificacao_query,
        (
            request.produto_id,
...
async def adicionar_golden_set_ncm(
    ...
    result = execute_query(
        "golden_set",
        insert_query,
        (produto_nome, produto_descricao, categoria, ncm_codigo),
    )
...
async def estatisticas_sistema():
    ...
    golden_cest_query = (
        "SELECT COUNT(*) as count FROM golden_set_cest WHERE ativo = TRUE"
    )
    ...
    return {
        ...
        "golden_set": {"ncm_items": total_golden_ncm, "cest_items": total_golden_cest},
