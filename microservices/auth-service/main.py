...
from shared.auth import AuthManager, get_current_user  # noqa: E402
...
async def login_user(user_data: UserLogin, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not user or not AuthManager.verify_password(
...
async def validate_token(current_user: dict = Depends(get_current_user)):
    """Validate JWT token"""
    return BaseResponse(message="Token is valid", data=current_user)
