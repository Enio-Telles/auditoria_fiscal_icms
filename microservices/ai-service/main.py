...
class AIInteraction(Base):
    ...
    interaction_metadata = Column(JSON)  # Renamed from 'metadata' to avoid SQLAlchemy conflict
    ...
    for provider_name, config in AI_PROVIDERS.items():
        try:
            if provider_name == "ollama":
                ...
                status = {"available": bool(config["api_key"]), "models": config["models"]}
...
async def generate_text(
    ...
    try:
        ...
        provider, model = await select_provider_and_model(request.provider, request.model)
        ...
        try:
            ...
            logger.info(f"Text generated for tenant {tenant_id} using {provider}/{model}")
...
async def chat_completion(
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Chat completion for tenant {tenant_id} using {provider}/{model}")
...
async def classify_product_ai(
    ...
    try:
        ...
        valid_categories = [
            "PRODUTO_ALIMENTICIO", "PRODUTO_ELETRONICO", "PRODUTO_TEXTIL",
            "PRODUTO_FARMACEUTICO", "PRODUTO_COSMETICO", "PRODUTO_INDUSTRIAL",
            "PRODUTO_AUTOMOTIVO", "PRODUTO_CONSTRUCAO", "PRODUTO_OUTROS"
        ]
...
async def select_provider_and_model(provider_request: str, model_request: Optional[str]) -> tuple:
...
async def call_ai_provider(
    ...
    if provider == "ollama":
        ...
        return await call_anthropic(model, prompt, system_prompt, max_tokens, temperature)
    else:
        # Mock response
        return {
...
async def call_chat_provider(
    ...
    return await call_ai_provider(provider, model, prompt, None, max_tokens, temperature)
...
async def call_ollama(model: str, prompt: str, system_prompt: Optional[str], max_tokens: int, temperature: float) -> Dict[str, Any]:
    """Call Ollama API"""
    try:
...
async def call_openai(model: str, prompt: str, system_prompt: Optional[str], max_tokens: int, temperature: float) -> Dict[str, Any]:
...
async def call_anthropic(model: str, prompt: str, system_prompt: Optional[str], max_tokens: int, temperature: float) -> Dict[str, Any]:
