...
async def get_available_strategies():
    ...
    return BaseResponse(
        message="Available classification strategies", data=CLASSIFICATION_STRATEGIES
    )
...
async def classify_product(
    ...
    try:
        ...
        classification_result = await perform_classification(
            request.descricao, request.estrategia, tenant_id
        )
        ...
        logger.info(
            "Product %s classified as %s for tenant %s",
            request.product_id,
            classification_result["classification"],
            tenant_id,
        )
    ...
    try:
        ...
        for product_data in request.products:
            try:
                ...
                # Perform classification
                classification_result = await perform_classification(
                    product_data["descricao"], request.estrategia, tenant_id
                )
                ...
                logger.error(
                    f"Error classifying product {product_data['product_id']}: {str(e)}"
                )
                results.append(
                    {
                        "product_id": product_data["product_id"],
...
async def get_classification_history(
    ...
    try:
        ...
        total = query.count()
        history = (
            query.order_by(ClassificationHistory.created_at.desc())
            .offset(skip)
            .limit(limit)
            .all()
        )
...
async def get_classification_statistics(
    ...
    try:
        total_classifications = (
            ...
            .filter(ClassificationHistory.tenant_id == tenant_id)
            .count()
        )
        ...
        strategy_stats = {}
        strategies = (
            db.query(ClassificationHistory.estrategia_usada)
            .filter(ClassificationHistory.tenant_id == tenant_id)
            .distinct()
            .all()
        )
        ...
        # Average confidence
        avg_confidence = (
            db.query(ClassificationHistory.confianca)
            .filter(
                ClassificationHistory.tenant_id == tenant_id,
                ClassificationHistory.status == "completed",
            )
        ...
        average_confidence = (
            sum([c[0] for c in avg_confidence if c[0]]) / len(avg_confidence)
            if avg_confidence
            else 0
        )
        ...
        return BaseResponse(
            ...
            data={
                ...
                "failed_classifications": total_classifications
                - successful_classifications,
                "success_rate": (
                    (successful_classifications / total_classifications * 100)
...
async def perform_classification(
    ...
    if any(
        ...
        for word in ["alimento", "comida", "bebida", "leite", "carne"]
    ):
        classificacao = "PRODUTO_ALIMENTICIO"
        confianca = 0.95
    elif any(
        word in descricao_lower
        for word in ["eletronico", "computador", "celular", "tv"]
    ):
        classificacao = "PRODUTO_ELETRONICO"
        confianca = 0.90
    elif any(
        word in descricao_lower for word in ["roupa", "camisa", "calca", "tecido"]
    ):
        classificacao = "PRODUTO_TEXTIL"
        confianca = 0.85
    elif any(
        word in descricao_lower for word in ["remedio", "medicamento", "farmacia"]
    ):
    ...
    return {
        ...
        "details": {
            "keywords_found": [
                word
                for word in ["alimento", "eletronico", "roupa", "remedio"]
                if word in descricao_lower
            ],
...
async def get_product_details(
    product_id: int, tenant_id: str
) -> Optional[Dict[str, Any]]:
    """Get product details from product service"""
    try:
        # Mock product service call - replace with actual HTTP call
        return {"id": product_id, "tenant_id": tenant_id, "exists": True}
    except Exception as e:
...
async def update_product_classification(
    product_id: int, classificacao: str, confianca: float, tenant_id: str
):
    """Update product with classification results"""
    try:
        # Mock product service update - replace with actual HTTP call
        logger.info(
            f"Product {product_id} classification updated: {classificacao} (confidence: {confianca})"
        )
        return True
    except Exception as e:
