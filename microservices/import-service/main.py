...
async def upload_file(
    ...
    try:
        ...
        try:
            ...
            # Apply settings (header_row not currently used explicitly)
            ...
            # Process data
            results = await process_import_data(
                df, import_job, import_settings, tenant_id, db
            )
            ...
            logger.info(
                f"Import job {job_id} completed: {results['successful']} successful, {results['failed']} failed"
            )
...
async def list_import_jobs(
    ...
    try:
        ...
        return BaseResponse(
            ...
            data={"jobs": job_list, "total": total, "skip": skip, "limit": limit},
        )
...
async def get_import_job(
    ...
    try:
        job = (
            db.query(ImportJob)
            .filter(ImportJob.job_id == job_id, ImportJob.tenant_id == tenant_id)
            .first()
        )
...
async def get_import_errors(
    ...
    try:
        ...
        return BaseResponse(
            ...
            data={"errors": error_list, "total": total, "skip": skip, "limit": limit},
        )
...
async def get_import_statistics(
    ...
    try:
        total_jobs = (
            db.query(ImportJob).filter(ImportJob.tenant_id == tenant_id).count()
        )
        ...
        failed_jobs = (
            db.query(ImportJob)
            .filter(ImportJob.tenant_id == tenant_id, ImportJob.status == "failed")
            .count()
        )

        # Total rows processed
        total_rows_query = (
            db.query(ImportJob.processed_rows)
            .filter(ImportJob.tenant_id == tenant_id)
            .all()
        )
    ...
    for expected_col, possible_names in expected_columns.items():
        ...
        if mapped_col and mapped_col in df.columns:
            ...
            for possible_name in possible_names:
                matching_cols = [
                    col for col in df.columns if possible_name.lower() in col.lower()
                ]
                if matching_cols:
    ...
    for index, row in df.iterrows():
        try:
            ...
            if "codigo_produto" in mapped_columns:
                product_data["codigo_produto"] = str(
                    row[mapped_columns["codigo_produto"]]
                )
            else:
            ...
            # Numeric fields
            if "valor_unitario" in mapped_columns and pd.notna(
                row[mapped_columns["valor_unitario"]]
            ):
                try:
                    product_data["valor_unitario"] = float(
                        row[mapped_columns["valor_unitario"]]
                    )
            ...
            # Validate data
            if not product_data.get("codigo_produto") or not product_data.get(
                "descricao"
            ):
...
async def create_product_via_service(
    product_data: Dict[str, Any], tenant_id: str
) -> bool:
    """Create product via product service (mock implementation)"""
    try:
        # Mock implementation - in real scenario, make HTTP call to product service
        logger.info(
            f"Creating product: {product_data['codigo_produto']} for tenant {tenant_id}"
        )
        return True
    except Exception as e:
