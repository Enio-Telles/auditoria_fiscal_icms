    ...
    try:
        ...
        for existing_id, existing_empresa in empresas_db.items():
            if existing_empresa.get("cnpj") == empresa.cnpj:
                raise HTTPException(
                    status_code=400, detail="CNPJ já cadastrado no sistema"
                )
...
async def listar_empresas(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro ao listar empresas: {str(e)}"
        )
...
async def alterar_status_empresa(empresa_id: str, status_data: dict):
    ...
    if novo_status not in ["ATIVA", "INATIVA", "SUSPENSA"]:
        raise HTTPException(
            status_code=400, detail="Status inválido. Use: ATIVA, INATIVA ou SUSPENSA"
        )
...
async def excluir_empresa(empresa_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro ao excluir empresa: {str(e)}"
        )
