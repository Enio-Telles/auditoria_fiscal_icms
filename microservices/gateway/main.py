...
async def root():
    return BaseResponse(
        ...
        data={"services": list(SERVICES.keys())}
    )
...
async def criar_empresa(empresa: EmpresaCreate):
    ...
    try:
        logger.info(f"Criando empresa via Gateway: {empresa.cnpj} - {empresa.razao_social}")
...
async def forward_request(
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            logger.info(f"Forwarded {request.method} {path} to {service_name} - Status: {response.status_code}")
